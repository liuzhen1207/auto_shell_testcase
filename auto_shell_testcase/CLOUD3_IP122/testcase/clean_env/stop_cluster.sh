...
bm1_ip=`cat ${conf_file}|grep ^bm1_ip=|awk -F '=' '{print $2}'`
bm2_ip=`cat ${conf_file}|grep ^bm2_ip=|awk -F '=' '{print $2}'`
...
sleep 1
# 强制 kill 两台机器的 App 进程
echo "正在 kill BM1: $bm1_ip 上的 App 进程..."
ssh root@${bm1_ip} "pkill -9 -f App" 2>/dev/null

echo "正在 kill BM2: $bm2_ip 上的 App 进程..."
ssh root@${bm2_ip} "pkill -9 -f App" 2>/dev/null

echo "✅ 两台机器 App 进程已全部 kill -9 完成！"
sleep 1
}
stop_cluster
