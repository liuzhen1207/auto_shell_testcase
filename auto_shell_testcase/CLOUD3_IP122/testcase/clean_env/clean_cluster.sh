...
do
  if ssh ${os_user_name}@${line} test -d ${db_dir}/data; then
     ssh ${os_user_name}@${line} "source /etc/profile;rm -rf ${db_dir}/data"
  fi
  if ssh ${os_user_name}@${line} test -d ${db_dir}/logs; then
     ssh ${os_user_name}@${line} "source /etc/profile;rm -rf ${db_dir}/logs"
  fi
...
while read line <&3
do
  if ssh ${os_user_name}@${line} test -d ${cn_db_dir}/data; then
     ssh ${os_user_name}@${line} "source /etc/profile;rm -rf ${cn_db_dir}/data"
  fi
  if ssh ${os_user_name}@${line} test -d ${cn_db_dir}/logs; then
     ssh ${os_user_name}@${line} "source /etc/profile;rm -rf ${cn_db_dir}/logs"
  fi
