...
{
#V2-754 【2.0.9.1】 【数据库CLI登录】 使用不存在的用户登录时 【DN抛出空指针异常：Cannot read the array length because "this.measurements" is null】
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u lily -e "show cluster;">${cur_dir}/tmp.out
check_res "804: The user lily does not exist" 1 "lily is not exist login"
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u liuzhen -e "show cluster;">${cur_dir}/tmp.out
check_res "804: The user liuzhen does not exist" 1 "lily is not exist login"

for i in {1..100}
do
# create user
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${v_sec_super_user} -e "CREATE USER lily_${i} 'TimechoDB@2021';">${cur_dir}/tmp.out
check_res "successfully" 1 "CREATE USER lily_${i}"
done
...
do
# create user
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${v_sec_super_user} -e "DROP USER lily_${i};">${cur_dir}/tmp.out
check_res "successfully" 1 "DROP USER lily_${i}"
done
...
create_user
   cur_client_num=0
	for j in {1..100}
	do
           ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u lily_${j} -e "show cluster;" &
           let cur_client_num++
           if [[ ${cur_client_num} -ge ${max_client_num} ]];then
              wait
              cur_client_num=0
           fi 
	done
