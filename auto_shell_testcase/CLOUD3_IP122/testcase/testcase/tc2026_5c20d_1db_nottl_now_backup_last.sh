...
{
    local table_out="${cur_dir}/show_tables_${backup_db_name}.out"

    run_table_sql "${backup_db_name}" "show tables;" "${table_out}" || return 1
...
function build_group_count_sql()
{
    ...
    echo "select device_id,count(s_0),count(s_1),count(s_2),count(s_3),count(s_4),count(s_5),count(s_6),count(s_7),count(s_8),count(s_9),count(s_10),count(s_11) from ${table_name} where time >= ${window_start_ms} and time <= ${window_end_ms} group by device_id order by device_id;"
}
function build_object_sample_sql()
{
    ...
    local window_end_ms=$3
    local device_id=$4
    echo "select time,device_id,read_object(s_11) from ${table_name} where time >= ${window_start_ms} and time <= ${window_end_ms} and device_id='${device_id}' order by time limit 1;"
}
function capture_object_sample_baseline()
{
    ...
    do
        local device_sql="select device_id from ${table_name} where time >= ${window_start_ms} and time <= ${window_end_ms} order by device_id,time limit 1;"
        ...
        if ! query_output_has_rows "${device_out_file}"; then
            log "ERROR" "${db_name}.${table_name} 在当前时间窗口内未找到可用于对象校验的设备"
            return 1
        fi
        ...
        object_sql=$(build_object_sample_sql "${table_name}" "${window_start_ms}" "${window_end_ms}" "${device_id}")
...
function verify_object_sample_after_restore()
{
    ...
    do
        ...
        object_sql=$(build_object_sample_sql "${table_name}" "${window_start_ms}" "${window_end_ms}" "${device_id}")
        run_table_sql "${db_name}" "${object_sql}" "${verify_dir}/post_restore/${db_tag}_${table_name}_object.raw" || return 1
        ...
        diff -u "${verify_dir}/baseline/${db_tag}_${table_name}_object.out" "${verify_dir}/post_restore/${db_tag}_${table_name}_object.out" > "${verify_dir}/post_restore/${db_tag}_${table_name}_object.diff" || return 1
    done
...
function wait_backup_window_ready()
{
    ...
    do
        ...
        do
            local min_ms
            if ! min_ms=$(query_min_time_ms "${backup_db_name}" "${table_name}"); then
                load_failed=1
                break
            fi
            ...
            log "INFO" "${backup_db_name}.${table_name} min(time): $(ms_to_iso "${min_ms}") (${min_ms})"
        done

        if [[ ${load_failed} -eq 0 && -n "${first_ms}" ]]; then
...
function capture_backup_baseline()
{
    ...
    for table_name in "${backup_table_names[@]}"
    do
        run_table_sql "${backup_db_name}" "$(build_group_count_sql "${table_name}" "${usr_window_start_ms}" "${usr_window_end_ms}")" "${verify_dir}/baseline/${backup_db_name}_${table_name}_count.raw" || return 1
...
function execute_object_backup()
{
    ...
    ssh -o StrictHostKeyChecking=no ${backup_target_user}@${backup_target_host} "rm -rf ${backup_remote_root} && mkdir -p ${backup_remote_root}/${backup_db_name}" >> "${log_file}" 2>&1 || return 1
...
function delete_first_hour_data()
{
    ...
    for table_name in "${backup_table_names[@]}"
    do
        run_table_sql "${backup_db_name}" "delete from ${table_name} where time >= ${usr_window_start_ms} and time <= ${usr_window_end_ms};" "${verify_dir}/post_delete/${backup_db_name}_${table_name}_delete.raw" || return 1
    done
    ...
    for table_name in "${backup_table_names[@]}"
    do
        run_table_sql "${backup_db_name}" "$(build_group_count_sql "${table_name}" "${usr_window_start_ms}" "${usr_window_end_ms}")" "${verify_dir}/post_delete/${backup_db_name}_${table_name}_count.raw" || return 1
        ...
        query_output_is_empty "${verify_dir}/post_delete/${backup_db_name}_${table_name}_count.out" || return 1
    done

    log "INFO" "首个时间分区数据删除完成，时间: $(date +'%Y-%m-%d %H:%M:%S')，${#backup_table_names[@]} 张表按 device_id 分组校验均为空"
}
function import_backup_data()
{
    ...
    ssh ${os_user_name}@${restore_host} "export JAVA_HOME=${java_home_remote}; ${db_dir}/tools/import-data.sh -h 172.20.70.119 -p 6667 -u ${db_user_name} -pw ${root_pw} -sql_dialect table -db ${backup_db_name} -ft tsfile -s ${backup_remote_root}/${backup_db_name} -os none -of none -tn 16" >> "${log_file}" 2>&1 &
...
function verify_imported_data()
{
    ...
    for table_name in "${backup_table_names[@]}"
    do
        run_table_sql "${backup_db_name}" "$(build_group_count_sql "${table_name}" "${usr_window_start_ms}" "${usr_window_end_ms}")" "${verify_dir}/post_restore/${backup_db_name}_${table_name}_count.raw" || return 1
        ...
        diff -u "${verify_dir}/baseline/${backup_db_name}_${table_name}_count.out" "${verify_dir}/post_restore/${backup_db_name}_${table_name}_count.out" > "${verify_dir}/post_restore/${backup_db_name}_${table_name}_count.diff" || return 1
    done
