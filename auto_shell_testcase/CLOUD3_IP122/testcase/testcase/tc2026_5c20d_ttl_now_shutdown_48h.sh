...
nohup bash ./start_bm_now_2d.sh '${arg}' > '${launcher_log}' 2>&1 < /dev/null &
sleep 1
pgrep -af \"start_bm_now_2d.sh ${arg}\" >/dev/null"
}
...
pgrep -af \"start_bm_now_2d.sh ${arg}\" >/dev/null && launcher_alive=1
...
function testcase()
{
	...
	do
		...
		echo "shutdown ${v_shutdown_ip1} ${v_shutdown_ip2} at ${v_stop_timestamp}"
                ssh ${os_user_name}@${v_shutdown_ip1} "shutdown -h now" 
                ssh ${os_user_name}@${v_shutdown_ip2} "shutdown -h now"
	        sleep 10
	        exec_jstack
		fi
	done
