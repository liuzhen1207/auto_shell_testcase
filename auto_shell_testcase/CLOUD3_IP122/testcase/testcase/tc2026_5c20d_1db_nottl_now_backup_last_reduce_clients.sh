...
bm_ips=()
backup_db_name="usr_sod0"
backup_table_names=()
declare -A backup_table_time_columns=()
declare -A backup_table_count_exprs=()
declare -A backup_table_object_columns=()
...
bk_log_file="${cur_dir}/${testdb}/backup_import_tsfile.log"
mkdir -p "${cur_dir}/${testdb}/"
ssl_str=""
backup_flag=0
backup_log_flag=0
v_warnNum=0
...
backup_pipe_expected_running=1
...
db_cli_pw_arg=""
cli_query_timeout_sec=3600
query_tmp_dir="/tmp/${SCRIPT_NAME%.*}_query_tmp"
...
if ! command -v java >/dev/null 2>&1 && [[ -x "${java_home_remote}/bin/java" ]]; then
...
log_backup_import() {
    ...
    local msg=$2
    local timestamp=$(date +'%Y-%m-%d %H:%M:%S')
    echo "[${timestamp}] [${level}] ${msg}" >> "${bk_log_file}"
    echo "[${timestamp}] [${level}] ${msg}"
}
...
configure_confignode() {
    ...
    ssh -o ConnectTimeout=10 "${os_user_name}@${node_ip}" bash -s <<EOF >> "${log_file}" 2>&1
        ...
        batch_set_sys_conf ".*last_cache_operation_on_load=.*" "last_cache_operation_on_load=CLEAN_DEVICE"
EOF
...
local max_wait_time=${1:-120}
   ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${db_cli_pw_arg} ${ssl_str} -h ${query_ip} -e "flush;">${cur_dir}/tmp.out 2>&1 || return 1
   ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${db_cli_pw_arg} ${ssl_str} -h ${query_ip} -e "show datanodes;">${cur_dir}/tmp.out 2>&1 || return 1
   cat ${cur_dir}/tmp.out |grep Running|awk -F "|" '{gsub(" ","");print $4}'>${cur_dir}/tmp1.out
   mv ${cur_dir}/tmp1.out ${cur_dir}/tmp.out
   if [[ ! -s "${cur_dir}/tmp.out" ]]; then
      log "ERROR" "show datanodes 未解析到 Running DN，无法等待日志同步"
      return 1
   fi
   ...
   while true
   do
   ssh ${os_user_name}@${line} "grep \"create a new\" ${db_dir}/logs/log_datanode_all.log|grep usr_sod0">${cur_dir}/tmp1.out
   last_time_str1=$(tail -n 1 "${cur_dir}/tmp1.out" | awk -F',' '{print $1}')
   last_timestamp1=$(date -d "$last_time_str1" +%s 2>/dev/null)
   if [[ -z "${last_timestamp1}" ]]; then
      last_timestamp=0
   else
...
function query_running_pipe_count()
{
    ...
    ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${db_cli_pw_arg} ${ssl_str} -h ${query_ip} -sql_dialect table -timeout ${cli_query_timeout_sec} -e "show pipes;">"${pipe_out}" 2>&1 || return 1
    tr -d '\r' < "${pipe_out}" | awk -F'|' '
        {
            state=$4
            gsub(/[[:space:]]/, "", state)
            if (toupper(state) == "RUNNING" || toupper(state) == "STOPPED") {
                count++
            }
        }
        END {
...
usr_count=\$(find '${backup_remote_root}/${backup_db_name}' -maxdepth 1 -type f 2>/dev/null | wc -l)
echo \"\${usr_count}\"
" 2>/dev/null
}
function wait_backup_pipes_running()
{
    ...
    do
        ...
        if ! running_pipe_count=$(query_running_pipe_count); then
            log_backup_import "WARN" "执行 show pipes 失败，${backup_pipe_check_interval} 秒后重试"
        else
            log_backup_import "INFO" "当前 show pipes Running 数量: ${running_pipe_count}"
            log_pipe_progress_snapshot
            if [[ ${running_pipe_count} -eq ${backup_pipe_expected_running} ]]; then
                log_backup_import "INFO" "备份任务创建成功：show pipes 为 Running ${backup_pipe_expected_running} 条"
                return 0
            fi
        fi

        if ! kill -0 ${backup_pid} 2>/dev/null; then
            log_backup_import "ERROR" "备份进程已退出，但 show pipes 未达到 Running ${backup_pipe_expected_running} 条"
        ...
        if [[ $((now_ts - start_ts)) -ge ${backup_pipe_create_timeout} ]]; then
            log_backup_import "ERROR" "等待备份任务创建超时：show pipes 未达到 Running ${backup_pipe_expected_running} 条"
            [[ -f "${cur_dir}/show_pipes.out" ]] && cat "${cur_dir}/show_pipes.out"
            return 1
        fi
    ...
    do
        ...
        if [[ ${running_pipe_count:-0} -gt 0 && $((now_ts - last_progress_check_ts)) -ge ${backup_progress_sample_interval} ]]; then
            ...
            if top_file_stats=$(query_remote_backup_top_file_count); then
                log "INFO" "远端备份顶层 tsfile 数量: ${backup_db_name}=${top_file_stats}"
            else
...
function run_table_sql()
{
    ...
    local sql_text=$2
    local out_file=$3
    ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${db_cli_pw_arg} ${ssl_str} -h ${query_ip} -sql_dialect table -timeout ${cli_query_timeout_sec} -e "use ${db_name}; ${sql_text}">${out_file} 2>&1
    return $?
}
function table_sql_should_retry()
{
    local out_file=$1
    grep -Eq \
        'Fail to connect to any config node|Error in calling method showDatabase|database is not specified|All replicas for region\[.*\] are not available' \
        "${out_file}"
}
function run_table_sql_with_retry()
{
    local db_name=$1
    local sql_text=$2
    local out_file=$3
    ...
    while true; do
        run_table_sql "${db_name}" "${sql_text}" "${out_file}"
        rc=$?
        ...
        if ! table_sql_should_retry "${out_file}"; then
            return ${rc}
        fi

        if [[ ${attempt} -ge ${max_attempts} ]]; then
            log "ERROR" "表 SQL 执行重试耗尽: db=${db_name}, sql=${sql_text}"
            return ${rc}
        fi

        log "WARN" "表 SQL 执行失败，疑似集群短暂不可用，第 ${attempt}/${max_attempts} 次重试前等待 ${retry_interval}s: db=${db_name}, sql=${sql_text}"
...
function load_backup_tables()
{
    local table_out="${query_tmp_dir}/show_tables_${backup_db_name}.out"

    run_table_sql_with_retry "${backup_db_name}" "show tables;" "${table_out}" || return 1

    mapfile -t backup_table_names < <(
        awk -F'|' '
        /^[[:space:]]*\|/ {
            table_name=$2
            gsub(/^[[:space:]]+|[[:space:]]+$/, "", table_name)
            lower_name=tolower(table_name)
    ...
    if [[ ${#backup_table_names[@]} -eq 0 ]]; then
        log "ERROR" "数据库 ${backup_db_name} 未解析到任何表"
        return 1
    fi

    log "INFO" "数据库 ${backup_db_name} 共发现 ${#backup_table_names[@]} 张表"
    return 0
}
function ensure_backup_tables_loaded()
{
    if [[ ${#backup_table_names[@]} -eq 0 ]]; then
        load_backup_tables || return 1
    fi
    return 0
}
function query_table_time_column_name()
{
    local db_name=$1
    local table_name=$2
    local out_file="${query_tmp_dir}/desc_${db_name}_${table_name}.out"

    run_table_sql_with_retry "${db_name}" "desc ${table_name};" "${out_file}" || return 1

    awk -F'|' '
    /^[[:space:]]*\|/ {
        column_name=$2
        category=$4
        gsub(/^[[:space:]]+|[[:space:]]+$/, "", column_name)
        gsub(/^[[:space:]]+|[[:space:]]+$/, "", category)
        if (column_name != "" && toupper(category) == "TIME") {
            print column_name
            exit
        }
    }' "${out_file}"
}
function query_table_count_exprs()
{
    local db_name=$1
    local table_name=$2
    local out_file="${query_tmp_dir}/desc_${db_name}_${table_name}.out"
    ...
    /^[[:space:]]*\|/ {
        ...
        category=$4
        gsub(/^[[:space:]]+|[[:space:]]+$/, "", column_name)
        gsub(/^[[:space:]]+|[[:space:]]+$/, "", data_type)
        gsub(/^[[:space:]]+|[[:space:]]+$/, "", category)
        if (column_name != "" && toupper(category) == "FIELD") {
            if (exprs != "") {
                exprs = exprs ","
            }
            exprs = exprs "count(" column_name ")"
        }
    }
    END {
        if (exprs != "") {
...
function query_table_object_column_name()
{
    local db_name=$1
    local table_name=$2
    local out_file="${query_tmp_dir}/desc_${db_name}_${table_name}.out"
    ...
    /^[[:space:]]*\|/ {
        ...
        gsub(/^[[:space:]]+|[[:space:]]+$/, "", data_type)
        gsub(/^[[:space:]]+|[[:space:]]+$/, "", category)
        upper_type=toupper(data_type)
        if (column_name != "" && toupper(category) == "FIELD" && (upper_type == "OBJECT" || upper_type == "BLOB")) {
            print column_name
            exit
        }
    }' "${out_file}"
}
function get_backup_table_time_column()
{
    local db_name=$1
    local table_name=$2
    local cache_key="${db_name}.${table_name}"
    local time_column="${backup_table_time_columns[${cache_key}]:-}"
    ...
    time_column=$(query_table_time_column_name "${db_name}" "${table_name}") || return 1
    if [[ -z "${time_column}" ]]; then
        log "ERROR" "${db_name}.${table_name} 未解析到 TIME 列"
        return 1
    fi

    backup_table_time_columns["${cache_key}"]="${time_column}"
    log "INFO" "${db_name}.${table_name} 时间列: ${time_column}" >&2
    printf '%s\n' "${time_column}"
}
function get_backup_table_count_exprs()
{
    local db_name=$1
    local table_name=$2
    local cache_key="${db_name}.${table_name}"
    local count_exprs="${backup_table_count_exprs[${cache_key}]:-}"
    ...
    count_exprs=$(query_table_count_exprs "${db_name}" "${table_name}") || return 1
    if [[ -z "${count_exprs}" ]]; then
        log "ERROR" "${db_name}.${table_name} 未解析到任何 FIELD 列"
        return 1
    fi

    backup_table_count_exprs["${cache_key}"]="${count_exprs}"
    log "INFO" "${db_name}.${table_name} count 校验列: ${count_exprs}" >&2
    printf '%s\n' "${count_exprs}"
}
function get_backup_table_object_column()
{
    local db_name=$1
    local table_name=$2
    local cache_key="${db_name}.${table_name}"
    local object_column="${backup_table_object_columns[${cache_key}]:-}"

    if [[ -n "${object_column}" ]]; then
        printf '%s\n' "${object_column}"
        return 0
    fi

    object_column=$(query_table_object_column_name "${db_name}" "${table_name}") || return 1
    if [[ -z "${object_column}" ]]; then
        log "ERROR" "${db_name}.${table_name} 未解析到 OBJECT/BLOB 列"
        return 1
    fi

    backup_table_object_columns["${cache_key}"]="${object_column}"
    log "INFO" "${db_name}.${table_name} 对象校验列: ${object_column}" >&2
    printf '%s\n' "${object_column}"
}
...
function build_group_count_sql()
{
    local table_name=$1
    local time_column=$2
    local window_start_ms=$3
    local window_end_ms=$4
    local count_exprs=$5
    echo "select device_id,${count_exprs} from ${table_name} where ${time_column} >= ${window_start_ms} and ${time_column} <= ${window_end_ms} group by device_id order by device_id;"
}
function build_object_sample_sql()
{
    ...
    local window_end_ms=$4
    local device_id=$5
    local object_column=$6
    echo "select ${time_column},device_id,read_object(${object_column}) from ${table_name} where ${time_column} >= ${window_start_ms} and ${time_column} <= ${window_end_ms} and device_id='${device_id}' order by ${time_column} limit 1;"
}
function capture_object_sample_baseline()
{
    ...
    local table_name
    ensure_backup_tables_loaded || return 1
    ...
    do
        ...
        local object_column=""
        local device_sql=""
        local device_raw_file="${verify_dir}/baseline/${db_tag}_${table_name}_object_device.raw"
        local device_out_file="${verify_dir}/baseline/${db_tag}_${table_name}_object_device.out"
        local device_id_file="${verify_dir}/baseline/${db_tag}_${table_name}_object_device.txt"
        local object_sql=""
        local object_raw_file="${verify_dir}/baseline/${db_tag}_${table_name}_object.raw"
        local object_out_file="${verify_dir}/baseline/${db_tag}_${table_name}_object.out"
        local device_id=""

        time_column=$(get_backup_table_time_column "${db_name}" "${table_name}") || return 1
        object_column=$(get_backup_table_object_column "${db_name}" "${table_name}") || return 1
        device_sql="select device_id from ${table_name} where ${time_column} >= ${window_start_ms} and ${time_column} <= ${window_end_ms} order by device_id,${time_column} limit 1;"
        run_table_sql_with_retry "${db_name}" "${device_sql}" "${device_raw_file}" || return 1
        normalize_query_output "${device_raw_file}" "${device_out_file}"
        if ! query_output_has_rows "${device_out_file}"; then
            rm -f "${device_id_file}" "${object_raw_file}" "${object_out_file}"
            log "INFO" "${db_name}.${table_name} 在当前时间窗口内基线结果为空，跳过对象校验"
            continue
        fi

        device_id=$(extract_first_device_id "${device_out_file}")
        if [[ -z "${device_id}" ]]; then
            log "ERROR" "${db_name}.${table_name} 无法从查询结果中解析对象校验设备"
            return 1
        fi
        printf '%s\n' "${device_id}" > "${device_id_file}"

        object_sql=$(build_object_sample_sql "${table_name}" "${time_column}" "${window_start_ms}" "${window_end_ms}" "${device_id}" "${object_column}")
        run_table_sql_with_retry "${db_name}" "${object_sql}" "${object_raw_file}" || return 1
        normalize_query_output "${object_raw_file}" "${object_out_file}"
        if ! query_output_has_rows "${object_out_file}"; then
            log "ERROR" "${db_name}.${table_name} 设备 ${device_id} 对象校验结果为空"
            return 1
        fi

        log "INFO" "${db_name}.${table_name} 对象校验设备: ${device_id}"
    done
...
function verify_object_sample_after_restore()
{
    ...
    do
        ...
        if [[ ! -f "${device_id_file}" ]]; then
            log "INFO" "${db_name}.${table_name} 基线阶段未记录对象校验样本，跳过恢复后对象校验"
            continue
        fi

        device_id=$(tr -d '\r' < "${device_id_file}")
        if [[ -z "${device_id}" ]]; then
            log "INFO" "${db_name}.${table_name} 基线阶段对象校验样本为空，跳过恢复后对象校验"
            continue
        fi
        ...
        object_column=$(get_backup_table_object_column "${db_name}" "${table_name}") || return 1
        object_sql=$(build_object_sample_sql "${table_name}" "${time_column}" "${window_start_ms}" "${window_end_ms}" "${device_id}" "${object_column}")
        run_table_sql_with_retry "${db_name}" "${object_sql}" "${verify_dir}/post_restore/${db_tag}_${table_name}_object.raw" || return 1
        normalize_query_output "${verify_dir}/post_restore/${db_tag}_${table_name}_object.raw" "${verify_dir}/post_restore/${db_tag}_${table_name}_object.out"
        if ! diff -u "${verify_dir}/baseline/${db_tag}_${table_name}_object.out" "${verify_dir}/post_restore/${db_tag}_${table_name}_object.out" > "${verify_dir}/post_restore/${db_tag}_${table_name}_object.diff"; then
            let fail_flag++
            log_backup_import "ERROR" "${db_name}.${table_name} 对象内容恢复后与基线不一致，详见 ${verify_dir}/post_restore/${db_tag}_${table_name}_object.diff"
        fi
    done
...
function query_min_time_ms()
{
    local db_name=$1
    local table_name=$2
    local out_file="${query_tmp_dir}/min_time_${db_name}_${table_name}.out"
    local time_column=""

    time_column=$(get_backup_table_time_column "${db_name}" "${table_name}") || return 1
    run_table_sql_with_retry "${db_name}" "select min(${time_column}) from ${table_name};" "${out_file}" || return 1
    local iso_time
    if awk -F'|' '
        /^[[:space:]]*\|/ {
            field=$2
            gsub(/^[[:space:]]+|[[:space:]]+$/, "", field)
            if (tolower(field) == "null") {
                found=1
                exit 0
            }
        }
...
window_scan_first_ms=""
window_scan_partition_start_ms=""
window_scan_cross_partition=0
window_scan_non_empty_count=0
function scan_backup_window_candidates()
{
    local scan_label=$1
    shift
    local table_name
    ...
    do
        local min_ms
        local time_column=""
        local table_partition_start_ms=""
        local min_rc=0

        time_column=$(get_backup_table_time_column "${backup_db_name}" "${table_name}") || return 1
        min_ms=$(query_min_time_ms "${backup_db_name}" "${table_name}")
        min_rc=$?
        if [[ ${min_rc} -eq 2 ]]; then
            log "INFO" "[${scan_label}] ${backup_db_name}.${table_name} 当前为空表，跳过"
            continue
        fi
        if [[ ${min_rc} -ne 0 ]]; then
            log "WARN" "[${scan_label}] ${backup_db_name}.${table_name} 查询最小时间失败"
            return 1
        fi

        table_partition_start_ms=$(align_to_partition_start_ms "${min_ms}")
        non_empty_count=$((non_empty_count + 1))
        if [[ -z "${first_ms}" || ${min_ms} -lt ${first_ms} ]]; then
            first_ms=${min_ms}
        fi
        if [[ -z "${partition_start_ms}" ]]; then
            partition_start_ms=${table_partition_start_ms}
        elif [[ ${table_partition_start_ms} -ne ${partition_start_ms} ]]; then
            cross_partition=1
        fi

        log "INFO" "[${scan_label}] ${backup_db_name}.${table_name} min(${time_column}): $(ms_to_iso "${min_ms}") (${min_ms})"
    done

    window_scan_first_ms=${first_ms}
    window_scan_partition_start_ms=${partition_start_ms}
    window_scan_cross_partition=${cross_partition}
    window_scan_non_empty_count=${non_empty_count}
    return 0
}
function wait_backup_window_ready()
{
    ...
    local sample_tables=()
    local full_scan_required=0
    local i
    local sample_index

    ensure_backup_tables_loaded || return 1
    total_tables=${#backup_table_names[@]}
    sample_count=${backup_window_sample_size}
    if [[ ${sample_count} -gt ${total_tables} ]]; then
        sample_count=${total_tables}
    fi
    for ((i=0; i<sample_count; i++))
    do
        sample_index=$((i * total_tables / sample_count))
        sample_tables+=("${backup_table_names[$sample_index]}")
    done
    log "INFO" "时间窗口预扫描采样 ${sample_count}/${total_tables} 张表: ${sample_tables[*]}"

    while true
    do
        local first_ms=""

        if ! scan_backup_window_candidates "sample" "${sample_tables[@]}"; then
            :
        else
            first_ms=${window_scan_first_ms}
            full_scan_required=0
            if [[ ${window_scan_non_empty_count} -eq 0 ]]; then
                log "INFO" "当前采样表均为空，30 秒后重试"
            elif [[ ${sample_count} -lt ${total_tables} && ${window_scan_cross_partition} -ne 0 ]]; then
                full_scan_required=1
                log "INFO" "采样表最小时间跨多个时间分区，回退全量扫描以确认首个时间分区"
            fi

            if [[ ${window_scan_non_empty_count} -eq 0 ]]; then
                :
            elif [[ ${full_scan_required} -eq 0 ]]; then
                log "INFO" "采样表最小时间均落在同一时间分区，直接使用采样结果确定备份窗口"
            elif scan_backup_window_candidates "full" "${backup_table_names[@]}"; then
                first_ms=${window_scan_first_ms}
                if [[ ${window_scan_non_empty_count} -eq 0 ]]; then
                    log "INFO" "当前全量表均为空，30 秒后重试"
                fi
            fi
        fi

        if [[ -n "${first_ms}" ]]; then
            usr_window_start_ms=$(align_to_partition_start_ms "${first_ms}")
            ...
            log "INFO" "${backup_db_name} 首个时间分区窗口: ${usr_window_start_iso} ~ ${usr_window_end_iso}"
            break
        fi
        now_wait=$(date +%s)
    ...
    local ready_ms=$((usr_window_start_ms + 3 * time_partition_interval_ms))
...
function capture_backup_baseline()
{
    ...
    do
        ...
        local count_exprs=""
        time_column=$(get_backup_table_time_column "${backup_db_name}" "${table_name}") || return 1
        count_exprs=$(get_backup_table_count_exprs "${backup_db_name}" "${table_name}") || return 1
        run_table_sql_with_retry "${backup_db_name}" "$(build_group_count_sql "${table_name}" "${time_column}" "${usr_window_start_ms}" "${usr_window_end_ms}" "${count_exprs}")" "${verify_dir}/baseline/${backup_db_name}_${table_name}_count.raw" || return 1
        normalize_query_output "${verify_dir}/baseline/${backup_db_name}_${table_name}_count.raw" "${verify_dir}/baseline/${backup_db_name}_${table_name}_count.out"
    done

    capture_object_sample_baseline "${backup_db_name}" "${usr_window_start_ms}" "${usr_window_end_ms}" "${backup_db_name}" || return 1

    log "INFO" "导出前基线结果已保存到 ${verify_dir}/baseline"
}
function execute_object_backup()
{
    log_backup_import "INFO" "开始执行对象数据备份，时间: $(date +'%Y-%m-%d %H:%M:%S')，远端目录: ${backup_target_user}@${backup_target_host}:${backup_remote_root}"
    log_backup_import "INFO" "before backup show regions"
    local show_out1="${cur_dir}/${testdb}/before_backup_show_regions_tree.out"
    local show_out2="${cur_dir}/${testdb}/before_backup_show_regions_table.out"
    local show_out3="${cur_dir}/${testdb}/before_backup_show_cluster.out"
    ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${db_cli_pw_arg} ${ssl_str} -h ${query_ip} -sql_dialect tree -timeout ${cli_query_timeout_sec} -e "show cluster;">"${show_out3}" 2>&1 || return 1
    ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${db_cli_pw_arg} ${ssl_str} -h ${query_ip} -sql_dialect tree -timeout ${cli_query_timeout_sec} -e "show confignodes;">>"${show_out3}" 2>&1 || return 1
    ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${db_cli_pw_arg} ${ssl_str} -h ${query_ip} -sql_dialect tree -timeout ${cli_query_timeout_sec} -e "show regions;">"${show_out1}" 2>&1 || return 1
    ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${db_cli_pw_arg} ${ssl_str} -h ${query_ip} -sql_dialect table -timeout ${cli_query_timeout_sec} -e "show regions;">"${show_out2}" 2>&1 || return 1

    ssh -o StrictHostKeyChecking=no ${backup_target_user}@${backup_target_host} "rm -rf ${backup_remote_root} && mkdir -p ${backup_remote_root}/${backup_db_name}" >> "${bk_log_file}" 2>&1 || return 1
    local v_bk_t1=$(date +%s)
    ${db_dir}/tools/tsfile-backup.sh \
      ...
      --target="${backup_remote_root}/${backup_db_name}/" \
      ...
      --plugin_jar "${db_dir}/pipe_plugin/tsfile-remote-sink-2.0.8-SNAPSHOT-jar-with-dependencies.jar" >> "${bk_log_file}" 2>&1 &
    local usr_backup_pid=$!

    log_backup_import "INFO" "对象数据导出已启动，60 秒后尝试抓取一次备份期间 jstack"
    wait_backup_pipes_running ${usr_backup_pid} || return 1
    sleep 60
    if kill -0 ${usr_backup_pid} 2>/dev/null; then
        exec_jstack
    else
        log_backup_import "INFO" "备份任务在 60 秒内已结束，跳过备份期间 jstack"
    fi
    ...
    local v_bk_t2=$(date +%s)
    local v_bk_elp=$((v_bk_t2-v_bk_t1))

    exec_jstack

    log_backup_import "INFO" "对象数据导出完成，时间: $(date +'%Y-%m-%d %H:%M:%S')，backup elapsed ${v_bk_elp} sec. 远端目录 ${backup_target_user}@${backup_target_host}:${backup_remote_root}"
}
function delete_first_hour_data()
{
    log_backup_import "INFO" "开始删除首个时间分区数据，时间: $(date +'%Y-%m-%d %H:%M:%S')"
    local table_name
    ...
    do
        local time_column=""
        time_column=$(get_backup_table_time_column "${backup_db_name}" "${table_name}") || return 1
        if ! run_table_sql_with_retry "${backup_db_name}" "delete from ${table_name} where ${time_column} >= ${usr_window_start_ms} and ${time_column} <= ${usr_window_end_ms};" "${verify_dir}/post_delete/${backup_db_name}_${table_name}_delete.raw"; then
            let fail_flag++
            delete_failed=1
            log_backup_import "ERROR" "${backup_db_name}.${table_name} 删除首个时间分区数据失败，继续执行后续流程"
        fi
    done

    ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${db_cli_pw_arg} ${ssl_str} -h ${query_ip} -e "flush;">>${log_file} 2>&1 || return 1
    sleep 10
    ...
    do
        ...
        time_column=$(get_backup_table_time_column "${backup_db_name}" "${table_name}") || return 1
        count_exprs=$(get_backup_table_count_exprs "${backup_db_name}" "${table_name}") || return 1
        run_table_sql_with_retry "${backup_db_name}" "$(build_group_count_sql "${table_name}" "${time_column}" "${usr_window_start_ms}" "${usr_window_end_ms}" "${count_exprs}")" "${verify_dir}/post_delete/${backup_db_name}_${table_name}_count.raw" || return 1
        normalize_query_output "${verify_dir}/post_delete/${backup_db_name}_${table_name}_count.raw" "${verify_dir}/post_delete/${backup_db_name}_${table_name}_count.out"
        if ! query_output_is_empty "${verify_dir}/post_delete/${backup_db_name}_${table_name}_count.out"; then
            let fail_flag++
            delete_failed=1
            log_backup_import "ERROR" "${backup_db_name}.${table_name} 删除后校验仍存在数据，继续执行后续流程"
        fi
    done

    if [[ ${delete_failed} -eq 0 ]]; then
        log_backup_import "INFO" "首个时间分区数据删除完成，时间: $(date +'%Y-%m-%d %H:%M:%S')，${#backup_table_names[@]} 张表按 device_id 分组校验均为空"
    else
        log_backup_import "WARN" "首个时间分区数据删除阶段存在失败，已累计 fail_flag 并继续后续流程"
    fi
}
function import_backup_data()
{
    log_backup_import "INFO" "开始导入备份数据，时间: $(date +'%Y-%m-%d %H:%M:%S')，目标主机: ${restore_host}"
    if [[ "${backup_target_host}" != "${restore_host}" ]]; then
        log_backup_import "ERROR" "当前脚本仅支持在备份机与导入机相同的场景下原地导入，backup_target_host=${backup_target_host}, restore_host=${restore_host}"
        return 1
    fi
    log_backup_import "INFO" "使用备份目录原地导入，不复制、不删除原备份数据，源目录: ${backup_remote_root}"
    local remote_root_pw
    remote_root_pw=$(printf '%q' "${root_pw}")

    ssh ${os_user_name}@${restore_host} "export JAVA_HOME=${java_home_remote}; export PATH=${java_home_remote}/bin:\$PATH; ${db_dir}/tools/import-data.sh -h ${restore_host} -p 6667 -u ${db_user_name} -pw ${remote_root_pw} -sql_dialect table -db ${backup_db_name} -ft tsfile -s ${backup_remote_root}/${backup_db_name} -os none -of none -tn 16" >> "${bk_log_file}" 2>&1 &
    local usr_import_pid=$!

    log_backup_import "INFO" "对象数据导入已启动，60 秒后尝试抓取一次导入期间 jstack"
    sleep 60
    if kill -0 ${usr_import_pid} 2>/dev/null; then
        exec_jstack
    else
        log_backup_import "INFO" "导入任务在 60 秒内已结束，跳过导入期间 jstack"
    fi
    ...
    sleep 10
    exec_jstack
    log_backup_import "INFO" "备份数据已在 ${restore_host} 本机导入完成，时间: $(date +'%Y-%m-%d %H:%M:%S')"
    log_backup_import "INFO" "after import show regions"
    local show_out1="${cur_dir}/${testdb}/after_import_show_regions_tree.out"
    local show_out2="${cur_dir}/${testdb}/after_import_show_regions_table.out"
    local show_out3="${cur_dir}/${testdb}/after_import_show_cluster.out"
    ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${db_cli_pw_arg} ${ssl_str} -h ${query_ip} -sql_dialect tree -timeout ${cli_query_timeout_sec} -e "show cluster;">"${show_out3}" 2>&1 || return 1
    ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${db_cli_pw_arg} ${ssl_str} -h ${query_ip} -sql_dialect tree -timeout ${cli_query_timeout_sec} -e "show confignodes;">>"${show_out3}" 2>&1 || return 1
    ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${db_cli_pw_arg} ${ssl_str} -h ${query_ip} -sql_dialect tree -timeout ${cli_query_timeout_sec} -e "show regions;">"${show_out1}" 2>&1 || return 1
    ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${db_cli_pw_arg} ${ssl_str} -h ${query_ip} -sql_dialect table -timeout ${cli_query_timeout_sec} -e "show regions;">"${show_out2}" 2>&1 || return 1
...
{
    log_backup_import "INFO" "开始导入后数据校验，时间: $(date +'%Y-%m-%d %H:%M:%S')"
    local table_name
    ensure_backup_tables_loaded || return 1
    ...
    do
        ...
        time_column=$(get_backup_table_time_column "${backup_db_name}" "${table_name}") || return 1
        count_exprs=$(get_backup_table_count_exprs "${backup_db_name}" "${table_name}") || return 1
        run_table_sql_with_retry "${backup_db_name}" "$(build_group_count_sql "${table_name}" "${time_column}" "${usr_window_start_ms}" "${usr_window_end_ms}" "${count_exprs}")" "${verify_dir}/post_restore/${backup_db_name}_${table_name}_count.raw" || return 1
        normalize_query_output "${verify_dir}/post_restore/${backup_db_name}_${table_name}_count.raw" "${verify_dir}/post_restore/${backup_db_name}_${table_name}_count.out"
        if ! diff -u "${verify_dir}/baseline/${backup_db_name}_${table_name}_count.out" "${verify_dir}/post_restore/${backup_db_name}_${table_name}_count.out" > "${verify_dir}/post_restore/${backup_db_name}_${table_name}_count.diff"; then
            let fail_flag++
            log_backup_import "ERROR" "${backup_db_name}.${table_name} 导入后 count 与基线不一致，详见 ${verify_dir}/post_restore/${backup_db_name}_${table_name}_count.diff"
        fi
    done

    verify_object_sample_after_restore "${backup_db_name}" "${usr_window_start_ms}" "${usr_window_end_ms}" "${backup_db_name}" || return 1

    log_backup_import "INFO" "导入后 count 与对象内容均和导出前一致，时间: $(date +'%Y-%m-%d %H:%M:%S')"
}
function stop_bm()
{
    local host
    ...
    if [[ -z "${v_testtime:-}" ]]; then
        log "INFO" "未启动 BM 或未设置 v_testtime，跳过停止 BM"
        return 0
    fi
    ...
    do
        run_benchmark_ssh "${host}" "
pkill -f 'cn.edu.tsinghua.iot.benchmark.App' 2>/dev/null || true
pkill -f '${bm_dir}/${bm_conf}/' 2>/dev/null || true
" >> "${log_file}" 2>&1 || true
    done
...
${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${db_cli_pw_arg} ${ssl_str} -h ${query_ip} -e "show cluster;">${cur_dir}/tmp.out 2>&1 || return 1
cat ${cur_dir}/tmp.out
check_res Running ${total_node_num} "show cluster expect ${total_node_num} Running but "
${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${db_cli_pw_arg} ${ssl_str} -h ${query_ip} -e "show regions;">${cur_dir}/tmp.out 2>&1 || return 1
cat ${cur_dir}/tmp.out
${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${db_cli_pw_arg} ${ssl_str} -h ${query_ip} -sql_dialect table -e "show regions;">${cur_dir}/tmp.out 2>&1 || return 1
cat ${cur_dir}/tmp.out
...
sh -x ${clean_env_dir}/backup_cluster_cn_data.sh ${v_tc_name_pre}_${v_backup_time}>> "${log_file}" 2>&1
}
# start cluster 
echo "">${log_file}
if [[ -n "${root_pw}" ]]; then
