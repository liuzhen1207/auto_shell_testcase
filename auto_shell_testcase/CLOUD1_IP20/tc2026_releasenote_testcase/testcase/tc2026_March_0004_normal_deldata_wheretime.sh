...
wait_for_sync_completion() {
    ...
    while true; do
        ...
        local response=$(curl -s -u "$PROMETHEUS_USER:$PROMETHEUS_PASS" "$PROMETHEUS_URL/api/v1/query?query=iot_consensus%7Bcluster%3D%22$CLUSTER_ID%22%2CnodeType%3D%22DATANODE%22%2Cname%3D%22ioTConsensusServerImpl%22%2Ctype%3D%22syncLag%22%7D")
