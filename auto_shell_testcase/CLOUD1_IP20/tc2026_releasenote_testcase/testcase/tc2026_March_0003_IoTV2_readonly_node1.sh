...
while true
do
${cli_dir}/sbin/start-cli.sh -h ${readonly_ip} -e "SET SYSTEM TO READONLY ON LOCAL;" 
while true
do
        v_jps=`${cli_dir}/sbin/start-cli.sh -h ${readonly_ip} -e "show cluster;"|grep "${readonly_ip}|"|grep -i only|wc -l`
        if [[ ${v_jps} -gt 0 ]];then
           break
        else
           sleep 10
...
sleep 60
${cli_dir}/sbin/start-cli.sh -h ${readonly_ip} -e "SET SYSTEM TO RUNNING ON LOCAL;" 
