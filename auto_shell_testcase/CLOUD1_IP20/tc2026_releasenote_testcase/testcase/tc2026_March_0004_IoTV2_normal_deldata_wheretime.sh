...
wait_for_sync_completion() {
    ...
    while true; do
        ...
        local response=$(curl -s -u "$PROMETHEUS_USER:$PROMETHEUS_PASS" "$PROMETHEUS_URL/api/v1/query?query=pipe_consensus%7Bcluster%3D%22$CLUSTER_ID%22%2CnodeType%3D%22DATANODE%22%2Cname%3D%22PipeConsensusServerImpl%22%2Ctype%3D%22syncLag%22%7D")

        # 检查响应是否成功
        if [[ $(echo "$response" | jq -r '.status') != "success" ]]; then
            echo "错误: 无法从 Prometheus 获取数据"
            echo "响应: $response"
            sleep 1
            continue
        fi

        # 检查是否所有 DataNode 的值都为 0
        local all_zero=true
        local result_count=$(echo "$response" | jq -r '.data.result | length')

        if [[ $result_count -eq 0 ]]; then
            echo "警告: 未找到 Total Sync Lag 指标数据"
            sleep 1
            continue
        fi
        ...
        for ((i=0; i<result_count; i++)); do
            local instance=$(echo "$response" | jq -r ".data.result[$i].metric.instance")
            local sync_lag=$(echo "$response" | jq -r ".data.result[$i].value[1]")
            local cluster=$(echo "$response" | jq -r ".data.result[$i].metric.cluster")
            local node_type=$(echo "$response" | jq -r ".data.result[$i].metric.nodeType")
            ...
            if (( $(echo "$sync_lag > 0.0001" | bc -l) )); then
                ...
                let fail_flag++
                let sync_fail_flag++
                v_warnMessage="sync lag > 0."
                echo "  ⚠️  实例 $instance 的 Total Sync Lag 大于 0，未达到同步完成条件"
                current_duration=0
                start_time=$(date +%s)
            fi
        done
...
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "use db_g_0 ;create or replace view table_0(device_id string tag) as root.test.g_0.**;"
   ...
   if [[ ${exception_num} -ge 10 ]];then
      ...
      if [[ ${v_diff_total} -gt 0 ]];then
         ...
         diff ${cur_dir}/q_all_online_table.out ${cur_dir}/q_stop_ip${v_ip}_table.out|grep "root"
...
EOF
    ...
    # 配置项：核心 - 限制最多2个delete并发
    local MAX_CONCURRENT=2
    local v_dev_num=49
    local PID_LIST=()  # 存储后台进程PID
    ...
    do
        ...
        for v_dev_idx in $(seq 0 $v_dev_num); do
            # 检查App进程是否存在，不存在则退出设备循环
            local v_bm=$(jps | grep App | wc -l)
            if [[ ${v_bm} -eq 0 ]]; then
                echo "App进程已退出，终止delete任务"
                break 2  # 跳出外层for循环（v_dev_idx）+ while循环
            fi
            ...
            do
                ...
                # ========== 查询test1最大时间并执行delete ==========
                local v_max_time1=$("${cli_dir}/sbin/start-cli.sh" -h "${query_ip}" -sql_dialect tree -timeout 3600 -e "select ${v_sensor_name} from root.test1.g_0.d_${v_dev_idx} order by time desc limit 1;" | grep "+08:00" | awk -F '|' '{print $2}')
                if [[ -n "${v_max_time1}" ]];then
                    # 后台执行delete
                    "${cli_dir}/sbin/start-cli.sh" -h "${query_ip}" -sql_dialect tree -timeout 3600 -e "delete from root.test1.g_0.d_${v_dev_idx}.${v_sensor_name} where time<=${v_max_time1};" &
                    PID_LIST+=($!)  # 记录PID

                    # ========== 兼容版：检查并发数，达到上限则等待任意一个进程完成 ==========
                    while [[ ${#PID_LIST[@]} -ge ${MAX_CONCURRENT} ]]; do
                        # 遍历PID列表，检查哪些进程还在运行
                        local NEW_PID_LIST=()
                        for pid in "${PID_LIST[@]}"; do
                            if ps -p "$pid" > /dev/null 2>&1; then
                                NEW_PID_LIST+=("$pid")  # 保留运行中的PID
                            fi
                        done
                        PID_LIST=("${NEW_PID_LIST[@]}")  # 更新PID列表（清理已完成的）
                        
                        # 如果仍达到上限，休眠0.5秒再检查
                        if [[ ${#PID_LIST[@]} -ge ${MAX_CONCURRENT} ]]; then
                            sleep 0.5
                        fi
                    done
                fi

                # ========== 查询test2最大时间并执行delete ==========
                local v_max_time2=$("${cli_dir}/sbin/start-cli.sh" -h "${query_ip}" -sql_dialect tree -timeout 3600 -e "select ${v_sensor_name} from root.test2.g_0.d_${v_dev_idx} order by time desc limit 1;" | grep "+08:00" | awk -F '|' '{print $2}')
                if [[ -n "${v_max_time2}" ]];then
                    # 后台执行delete
                    "${cli_dir}/sbin/start-cli.sh" -h "${query_ip}" -sql_dialect tree -timeout 3600 -e "delete from root.test2.g_0.d_${v_dev_idx}.${v_sensor_name} where time<=${v_max_time2};" &
                    PID_LIST+=($!)  # 记录PID
        ...
        if [[ ${#PID_LIST[@]} -gt 0 ]]; then
            wait "${PID_LIST[@]}"
            echo "本轮所有delete任务执行完成，当前剩余PID数：${#PID_LIST[@]}"
        else
            echo "本轮无需要执行的delete任务"
        fi

        # 检查App进程，不存在则退出while循环（终止函数）
        v_bm=$(jps | grep App | wc -l)
        if [[ ${v_bm} -eq 0 ]]; then
            echo "App进程已退出，del_data函数终止"
            return 0
        fi
