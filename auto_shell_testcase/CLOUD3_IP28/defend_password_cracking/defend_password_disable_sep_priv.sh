...
function check_npe()
{
   ...
   v_err_num=`ssh ${os_name}@${db_ip} "grep \"AuthException: Incorrect password\" ${db_dir}/logs/*all*|wc -l"`
...
function testcase2()
{
   ...
   # 6th attempt from remote IP - account @ shell  client is locked but global is not lock
   ssh ${remote_cli_os_user}@${remote_cli_ip} "${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "801: Authentication failed" 1 "repeate_testcase2_6th"
   # 7th attempt - account @ shell  client is locked but global is not lock 
   ssh ${remote_cli_os_user}@${remote_cli_ip} "${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "801: Authentication failed" 1 "repeate_testcase2_7th"

#   javadi@ip13 right password ,login failed (account locked)
  ssh ${remote_cli_os_user}@${remote_cli_ip} "${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e \"show cluster;\"">${current_dir}/tmp.out
# update log
   check_res "822: Account is blocked due to consecutive failed logins" 1 "testcase2"

  ssh ${remote_cli_os_user}@${remote_cli_ip} "${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -e \"show cluster;\"">${current_dir}/tmp.out
# update log
   check_res "822: Account is blocked due to consecutive failed logins" 1 "repeate_testcase2"

# ip28 javadi login failed (account locked)
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;" >${current_dir}/tmp.out
# update log
   check_res "822: Account is blocked due to consecutive failed logins" 1 "testcase2"
# db_ip javadi login failed (account locked)
...
# remove client ip ip13
  ssh ${remote_cli_os_user}@${remote_cli_ip} "${cli_dir}/sbin/start-cli.sh -u ${other_user} -h ${query_ip} -pw TimechoDB@2021 -e \"show cluster;\"">${current_dir}/tmp.out
  check_res "Running" ${node_num} "repeate_testcase2"
...
function testcase3()
{
   ...
   # 6th attempt from remote IP - not lock the account
   ssh ${remote_cli_os_user}@${remote_cli_ip} "${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "801: Authentication failed" 1 "repeate_testcase3_6th"
   # 7th attempt - remote not locked 
   ssh ${remote_cli_os_user}@${remote_cli_ip} "${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "801: Authentication failed" 1 "repeate_testcase3_7th"
...
function testcase5()
{
   ...
   for i in {1..3}
   do
      ssh ${remote_cli_os_user}@${remote_cli_ip} "${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw wrongpassword -e \"show cluster;\"">>${current_dir}/tmp.out
...
# unlock santos
   ...
   ${cli_dir}/sbin/start-cli.sh -u ${other_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "Running" ${node_num} "repeate_testcase5"
   ssh ${remote_cli_os_user}@${remote_cli_ip} "${cli_dir}/sbin/start-cli.sh -u ${other_user} -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "repeate_testcase5"
...
# remote client ip right password
   ssh ${remote_cli_os_user}@${remote_cli_ip} "${cli_dir}/sbin/start-cli.sh -u ${test_user} -pw TimechoDB@2021 -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "repeate_testcase7"
...
function testcase8()
{
   ...
   echo "">${current_dir}/tmp.out
   # 10th attempt - will fail and lock the account
   ssh ${os_name}@${db_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -pw wrongpassword -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Authentication failed" 1 "repeate_testcase8_10th"
   # 11th attempt - account already locked
   ssh ${os_name}@${db_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -pw wrongpassword -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "822: Account is blocked due to consecutive failed logins" 1 "repeate_testcase8_11th"
   echo "">${current_dir}/tmp.out
...
function testcase12()
{
   ...
   remove_data_logs
   set_params 5 -1 10
   start_dn_cn_exp_error "Login lock manager initialized with"
   test_user=javadi
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -e "create user ${test_user} 'TimechoDB@2021';">${current_dir}/tmp.out
  ...
  # remote cli ip failed 4
   ...
   for i in {1..4}
   do
   ssh ${remote_cli_os_user}@${remote_cli_ip} "${cli_dir}/sbin/start-cli.sh -u ${test_user} -pw wrongpassword -h ${query_ip}  -e \"show cluster;\"">>${current_dir}/tmp.out
   done
   check_res "Authentication failed" 4 "repeate_testcase12"
   echo "">${current_dir}/tmp.out
  # is not locked
...
function testcase13()
{
   ...
   remove_data_logs
   set_params 5 0 10
   start_dn_cn_exp_error "Login lock manager initialized with: IP-level attempts=5, User-level attempts=1000, Lock time=10 minutes"
   test_user=javadi
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -e "create user ${test_user} 'TimechoDB@2021';">${current_dir}/tmp.out
...
function testcase15()
{
   ...
   # this ip failed - 1st attempt (will fail but not lock yet)
   echo "">${current_dir}/tmp.out
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB -e "show cluster;">>${current_dir}/tmp.out
   check_res "Authentication failed" 1 "repeate_testcase15_1st"
   # 2nd attempt - should be locked now
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB -e "show cluster;">>${current_dir}/tmp.out
   check_res "822: Account is blocked due to consecutive failed logins" 1 "repeate_testcase15_2nd"
   # dn ip right password failed 2 (account locked by IP-level, not user-level)
   echo "">${current_dir}/tmp.out
   for i in {1..2}
   do
   ssh ${os_name}@${db_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -pw TimechoDB@2021 -h ${query_ip}  -e \"show cluster;\"">>${current_dir}/tmp.out
  ...
  # right pass remote cli ip failed 4 (account locked by IP-level)
   echo "">${current_dir}/tmp.out
   for i in {1..4}
   do
   ssh ${remote_cli_os_user}@${remote_cli_ip} "${cli_dir}/sbin/start-cli.sh -u ${test_user} -pw TimechoDB@2021 -h ${query_ip}  -e \"show cluster;\"">>${current_dir}/tmp.out
...
echo "NPE ${log_npe_flag},OTHER FAILED TESTCASE ${repeate_fail_flag}"
