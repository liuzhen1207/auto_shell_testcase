...
# enable audit
   v_param_exist=`ssh ${os_name}@${db_ip} "grep enable_audit_log= ${db_dir}/conf/iotdb-system.properties|wc -l"`
   v_param_exist_support=`ssh ${os_name}@${db_ip} "grep enable_audit_log ${db_dir}/conf/iotdb-system.properties.template|wc -l"`
   if [[ ${v_param_exist_support} -gt 0 ]];then
           if [[ ${v_param_exist} = 0 ]];then
             ssh ${os_name}@${db_ip} "echo enable_audit_log=true>> ${db_dir}/conf/iotdb-system.properties"
           else
             ssh ${os_name}@${db_ip} "sed -i 's/enable_audit_log=.*/enable_audit_log=true/g' ${db_dir}/conf/iotdb-system.properties"
           fi
   fi
...
echo "flush;" >>${current_dir}/exec_sql.sql
done
...
# enable_separation_of_powers=false ,root, enable_black_list=true,add this ip to blacklist 
function testcase1()
{
   stop_dn_cn
   remove_data_logs
...
#blacklist unset
   ...
   start_dn_cn
# create user santos 
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -e "create user santos 'TimechoDB@2021';" >${current_dir}/tmp.out
   ...
   check_res "success" 1 "repeate_testcase1"
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -sql_dialect table -e "grant all on any to user santos;">${current_dir}/tmp.out
   check_res "success" 1 "repeate_testcase1"
...
# root at this ip  put this ip to blacklist 
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -sql_dialect table -e "show cluster;set configuration black_ip_list='${this_shell_ip}';load configuration;show cluster;">${current_dir}/tmp.out
   check_res "success" 1 "repeate_testcase1"
   sleep 3
   check_res "802: Log in failed. Either you are not authorized or the session has timed out" 2 "testcase1"
...
# enable_separation_of_powers=false ,root, enable_white_list=true,remove this ip from whilelist
function testcase2()
{
   stop_dn_cn
   remove_data_logs
...
# root at this ip  put this ip to blacklist
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -sql_dialect table -e "show cluster;set configuration white_ip_list='${remote_cli_ip}';load configuration;show cluster;">${current_dir}/tmp.out
   check_res "success" 1 "repeate_testcase2"
   sleep 3
   check_res "802: Log in failed. Either you are not authorized or the session has timed out" 2 "testcase2"
   v_log_fail_num=`grep "802: Log in failed. Either you are not authorized or the session has timed out" ${current_dir}/exec_sql.out|wc -l`
   if [[ ${v_log_fail_num} -gt 0 ]];then
      let repeate_succ_flag++
   else
...
# enable_separation_of_powers=false ,root, enable_white_list=true,and then enable_white_list=false
function testcase3()
{
   stop_dn_cn
   remove_data_logs
...
# root at this ip  put this ip to blacklist
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -sql_dialect table -e "show cluster;set CONFIGURATION enable_white_list='false';load configuration;show cluster;">${current_dir}/tmp.out
   check_res "success" 2 "repeate_testcase3"
   sleep 3
...
# enable_separation_of_powers=false ,drop user
function testcase4()
{
   stop_dn_cn
   remove_data_logs
...
# root at this ip  put this ip to blacklist
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -sql_dialect table -e "drop user santos;">${current_dir}/tmp.out
   check_res "success" 1 "repeate_testcase4"
   sleep 3
   v_log_fail_num=`grep "802: Log in failed. Either you are not authorized or the session has timed out" ${current_dir}/exec_sql.out|wc -l`
   if [[ ${v_log_fail_num} -gt 0 ]];then
...
# enable_separation_of_powers=false ,alter user password
function testcase5()
{
   stop_dn_cn
   remove_data_logs
...
# root at this ip  put this ip to blacklist
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -sql_dialect table -e "alter user santos set password 'TimechoDB@2022';">${current_dir}/tmp.out
   check_res "success" 1 "repeate_testcase5"
   sleep 3
   v_log_fail_num=`grep "802: Log in failed. Either you are not authorized or the session has timed out" ${current_dir}/exec_sql.out|wc -l`
   if [[ ${v_log_fail_num} -gt 0 ]];then
   ...
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -sql_dialect table -e "show cluster;alter user root set password 'TimechoDB@2022';show cluster;">${current_dir}/tmp.out
   check_res "802: Log in failed. Either you are not authorized or the session has timed out" 1 "repeate_testcase5"
   check_npe "testcase5"
}
# enable_separation_of_powers=false ,idle_session_timeout_in_minutes=0
function testcase6()
{
   stop_dn_cn
   remove_data_logs
...
# enable_separation_of_powers=false ,idle_session_timeout_in_minutes=1
function testcase7()
{
   stop_dn_cn
   remove_data_logs
...
# enable_separation_of_powers=false ,idle_session_timeout_in_minutes=2
function testcase9()
{
   stop_dn_cn
   remove_data_logs
...
# enable_separation_of_powers=false ,idle_session_timeout_in_minutes=-1 ,hot load idle_session_timeout_in_minutes=1
function testcase10()
{
   stop_dn_cn
   remove_data_logs
...
# create user santos
   ...
   check_res "success" 1 "repeate_testcase10"
# admin hot load
${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -sql_dialect table -e "set CONFIGURATION idle_session_timeout_in_minutes='1';load configuration;">${current_dir}/tmp.out
check_res "success" 2 "repeate_testcase10"
# santos cli
    # 核心逻辑：通过管道向 CLI 输入命令，延迟在管道中处理
{
  # 等待 CLI 启动并连接（约1-2秒，根据实际情况调整）
...
# admin hot load
${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -sql_dialect table -e "set CONFIGURATION idle_session_timeout_in_minutes='-1';load configuration;">${current_dir}/tmp.out
check_res "success" 2 "repeate_testcase11"
# santos cli
    # 核心逻辑：通过管道向 CLI 输入命令，延迟在管道中处理
{
  # 等待 CLI 启动并连接（约1-2秒，根据实际情况调整）
