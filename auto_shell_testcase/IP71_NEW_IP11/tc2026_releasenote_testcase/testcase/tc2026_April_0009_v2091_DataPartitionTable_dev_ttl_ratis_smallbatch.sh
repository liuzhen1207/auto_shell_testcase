...
configure_confignode() {
    ...
    ssh -o ConnectTimeout=10 "${os_user_name}@${node_ip}" bash -s <<EOF >> "${log_file}" 2>&1
        ...
        batch_set_sys_conf ".*data_region_consensus_protocol_class=.*" "data_region_consensus_protocol_class=org.apache.iotdb.consensus.ratis.${v_consensus}"
...
bm_conf=ttl_partition_seq_ratis
#get root password
...
#wait_for_sync_completion
   ...
   sql1="select count(s_0),count(s_11),count(s_22),count(s_33),count(s_44),count(s_55),count(s_66),count(s_77),count(s_88),count(s_99) from root.test.g_0.** align by device;"
   sql2="select device_id,count(s_0),count(s_11),count(s_22),count(s_33),count(s_44),count(s_55),count(s_66),count(s_77),count(s_88),count(s_99) from db_g_0.table1_0 group by device_id order by device_id;"
   sql3="select device_id,count(s_0),count(s_11),count(s_22),count(s_33),count(s_44),count(s_55),count(s_66),count(s_77),count(s_88),count(s_99) from db_g_0.table2_0 group by device_id order by device_id;"
   ...
   sql8="select count(s_0),count(s_11),count(s_22),count(s_33),count(s_44),count(s_55),count(s_66),count(s_77),count(s_88),count(s_99) from root.test.g_0.** where time>=1970-01-09T09:00:03.000+08:00 and time<=2026-01-22T10:46:42.000+08:00 align by device;"
   sql9="select device_id,count(s_0),count(s_11),count(s_22),count(s_33),count(s_44),count(s_55),count(s_66),count(s_77),count(s_88),count(s_99) from db_g_0.table1_0 where time>=1970-01-09T09:00:03.000+08:00 and time<=2026-01-22T10:46:42.000+08:00 group by device_id order by device_id;"
   sql10="select device_id,count(s_0),count(s_11),count(s_22),count(s_33),count(s_44),count(s_55),count(s_66),count(s_77),count(s_88),count(s_99) from db_g_0.table2_0 where time>=1970-01-09T09:00:03.000+08:00 and time<=2026-01-22T10:46:42.000+08:00 group by device_id order by device_id;"
   # all online
   exception_num=0
   while true
   do
...
do
   ...
   result=$(ssh ${os_user_name}@${line} "du -sh ${db_dir}/data/datanode/consensus | awk '{print \$1}'") 
# 转换成 M 单位（核心：统一转成 MB 再判断）
size_m=$(echo "$result" | awk '
    /G/{printf "%.0f", $1*1024; exit}
    /M/{printf "%.0f", $1; exit}
    /K/{printf "0"; exit}
    /T/{printf "%.0f", $1*1024*1024; exit}
')

# 判断是否 > 500M
if (( size_m > 500 )); then
    echo "【失败】目录 data/datanode/consensus 大小为 $result（超过 500M）"
    let fail_flag++
    v_warnMessage="${v_warnMessage}DN ${line} data consensus size too large ${size_m} MB."
else
    echo "【正常】目录 data/datanode/consensus 大小为 $result"
fi
   if [[ ${v_npe} -gt 0 ]];then
           let fail_flag++
#	   let backup_log_flag++
