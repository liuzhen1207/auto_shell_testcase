...
license_dir=${db_parent_dir}/license_2026/
v_start_time=`date +%s`
...
echo "license is exist."
   ...
   if ssh ${os_user_name}@${line} test -f ${license_dir}/license; then
      ssh ${os_user_name}@${line} "sudo cp -rp ${license_dir}/license ${db_dir}/activation/license"
...
while true
do
   v_running=`${cli_dir}/sbin/start-cli.sh -h ${query_host} -e "show cluster;"|grep Running |wc -l`
   v_readonly=`${cli_dir}/sbin/start-cli.sh -h ${query_host} -e "show cluster;"|grep ReadOnly |wc -l`
