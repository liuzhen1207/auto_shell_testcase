...
# failed_login_attempts_per_user=7
# password_lock_time_minutes=2 
set -uo pipefail
...
configure_confignode() {
    ...
    ssh -o ConnectTimeout=10 "${os_user_name}@${node_ip}" bash -s <<EOF >> "${log_file}" 2>&1
        ...
        batch_set_sys_conf ".*failed_login_attempts_per_user=.*" "failed_login_attempts_per_user=7"
        batch_set_sys_conf ".*password_lock_time_minutes=.*" "password_lock_time_minutes=2"
EOF
...
function testcase1()
{
   ...
   check_res "801: Authentication failed" 5 "${test_user} is locked and exec show cluster;"
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${test_user} is locked and exec show cluster;"
   cat ${current_dir}/tmp.out
   # javadi@ip13 wrong password
   for i in {7..8}
   do
      ssh ${remote_cli_os_user}@${remote_cli_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB -e \"show cluster;\"">${current_dir}/tmp.out
      check_res "Authentication failed" 1 "${test_user}@${remote_cli_ip} input wrong password"
      cat ${current_dir}/tmp.out
   done

#   javadi@remove right password ,login failed
  ssh ${remote_cli_os_user}@${remote_cli_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e \"show cluster;\"">${current_dir}/tmp.out
# update log
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${test_user}@${remote_cli_os_user} right password is locked"
   cat ${current_dir}/tmp.out
  ssh ${remote_cli_os_user}@${remote_cli_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -e \"show cluster;\"">${current_dir}/tmp.out
# update log
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${test_user}@${remote_cli_os_user} right default password is locked"
   cat ${current_dir}/tmp.out
...
# update log
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${test_user}@client shell ip input right password"
   cat ${current_dir}/tmp.out
# db_ip javadi login failed
  db_ip=${query_ip}
  ssh ${os_name}@${db_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -e \"show cluster;\"">${current_dir}/tmp.out
# update log
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${test_user}@${db_ip} input right default password"
   cat ${current_dir}/tmp.out
...
# ip28
   ...
   check_res "Running" ${node_num} "${other_user} is not locked"
   cat ${current_dir}/tmp.out
# db_ip
  ssh ${os_name}@${db_ip} "${db_dir}/sbin/start-cli.sh -u ${other_user} -h ${query_ip} -pw TimechoDB@2021 -e \"show cluster;\"">${current_dir}/tmp.out
  check_res "Running" ${node_num} "${other_user}@${db_ip} is not locked"
   cat ${current_dir}/tmp.out
# remote client ip 
  ssh ${remote_cli_os_user}@${remote_cli_ip} "${db_dir}/sbin/start-cli.sh -u ${other_user} -h ${query_ip} -pw TimechoDB@2021 -e \"show cluster;\"">${current_dir}/tmp.out
  check_res "Running" ${node_num} "${other_user}@${remote_cli_ip} remote client ip"
# drop user
...
function testcase2()
{
   ...
   check_res "Authentication failed" 5 "${test_user} wrong password 6 times"
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${test_user} is locked"
   # javadi@ip13 wrong password
   for i in {7..8}
   do
      ssh ${remote_cli_os_user}@${remote_cli_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB -e \"show cluster;\"">${current_dir}/tmp.out
...
# ip28
   ${cli_dir}/sbin/start-cli.sh -u ${other_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;" >${current_dir}/tmp.out
   check_res "Running" ${node_num} "${other_user}@shell client right password login success"
# db_ip
  ssh ${os_name}@${db_ip} "${db_dir}/sbin/start-cli.sh -u ${other_user} -h ${query_ip} -pw TimechoDB@2021 -e \"show cluster;\"">${current_dir}/tmp.out
  check_res "Running" ${node_num} "${other_user}@${db_ip} right password login success"
# remote client ip ip13
  ssh ${remote_cli_os_user}@${remote_cli_ip} "${db_dir}/sbin/start-cli.sh -u ${other_user} -h ${query_ip} -pw TimechoDB@2021 -e \"show cluster;\"">${current_dir}/tmp.out
  check_res "Running" ${node_num} "${other_user}@${remote_cli_ip} right password login success"
# wait lock time login success
sleep 121

#   javadi@ip13 right password ,login failed
  ssh ${remote_cli_os_user}@${remote_cli_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${test_user} after lock time 120s login"

  ssh ${remote_cli_os_user}@${remote_cli_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${test_user}@${remote_cli_ip} after lock time 120s login"

   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;" >${current_dir}/tmp.out
  check_res "Running" ${node_num} "${test_user}@shell client after lock time 120s login"
  ssh ${os_name}@${db_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -e \"show cluster;\"">${current_dir}/tmp.out
  check_res "Running" ${node_num} "${test_user}@${db_ip} client after lock time 120s login"
   ...
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${test_user} is locked"
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${test_user}@shell client is locked"
   sleep 121
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${test_user}@shell client after lock time 120s login"
...
function testcase3()
{
  # create javadi
   ...
   check_res "Authentication failed" 5 "${test_user} wrong password 5 times"
# rightp password ,but is locked
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">>${current_dir}/tmp.out
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${test_user} is locked"
   # javadi@ip13 wrong password
# unlock user@ip
   ...
   check_res "success" 1 "${db_admin_name} unlock ${test_user} @ '${this_shell_ip}'"
# rightp password , after unlocked
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">>${current_dir}/tmp.out
   check_res "Running" ${node_num} "${test_user} after unlocked login"
# unlock santos@ip
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -pw TimechoDB@2021 -sql_dialect tree -e "alter user ${other_user} @ '${this_shell_ip}' account unlock;">${current_dir}/tmp.out
   check_res "success" 1 "${db_admin_name} unlock not-locked user ${other_user} @ '${this_shell_ip}'"

   # right password
   ${cli_dir}/sbin/start-cli.sh -u ${other_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${other_user} after unlock login"
   echo "">${current_dir}/tmp.out
   for i in {1..5}
   do
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB -e "show cluster;">>${current_dir}/tmp.out
   ...
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${test_user} is locked"
# unlock user@ip
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -pw TimechoDB@2021 -sql_dialect table -e "alter user ${test_user} @ '${this_shell_ip}' account unlock;">${current_dir}/tmp.out
   check_res "success" 1 "${db_admin_name} unlock ${test_user} @ '${this_shell_ip}'"
   # right password
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${test_user} after unlocked login"
# unlock santos@ip
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -pw TimechoDB@2021 -sql_dialect table -e "alter user ${other_user} @ '${this_shell_ip}' account unlock;">${current_dir}/tmp.out
   check_res "success" 1 "${db_admin_name} unlock not-locked user ${other_user} @ '${this_shell_ip}'"
   # right password
   ${cli_dir}/sbin/start-cli.sh -u ${other_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${other_user} after unlock login"
...
function testcase4()
{
  # create javadi
   ...
   check_res "Authentication failed" 4 "${test_user}@${this_shell_ip} wrong password 4 times"
   echo "">${current_dir}/tmp.out
   for i in {1..3}
   do
      ssh ${remote_cli_os_user}@${remote_cli_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw wrongpassword -e \"show cluster;\"">>${current_dir}/tmp.out
   done
   check_res "Authentication failed" 3 "${test_user}@${remote_cli_ip} wrong password 3 times"
# rightp password ,but is locked
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">>${current_dir}/tmp.out
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${test_user} is locked"
   ssh ${remote_cli_os_user}@${remote_cli_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "822: Account is blocked due to consecutive failed logins" 1 "global ${test_user} is locked at ${remote_cli_ip} login"
   ssh ${os_name}@${db_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -pw TimechoDB@2021 -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "822: Account is blocked due to consecutive failed logins" 1 "global ${test_user} is locked at ${db_ip} login"
# unlock user
   ...
   check_res "success" 1 "${db_admin_name} unlock ${test_user}"
# right password , after unlocked
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">>${current_dir}/tmp.out
   check_res "Running" ${node_num} "${test_user} after unlocked login"
# unlock santos@ip
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -pw TimechoDB@2021 -sql_dialect tree -e "alter user ${other_user} account unlock;">${current_dir}/tmp.out
   check_res "success" 1 "${db_admin_name} unlock not-locked user global ${other_user}"

   # right password
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "Running" ${node_num} "after unlock ${test_user} at shell ip login"
   ssh ${remote_cli_os_user}@${remote_cli_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "after unlock ${test_user}@${remote_cli_ip} login"
   ssh ${os_name}@${db_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -pw TimechoDB@2021 -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "after unlock ${test_user}@${db_ip} login"

   # right password
   ${cli_dir}/sbin/start-cli.sh -u ${other_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${other_user} login"
   ssh ${remote_cli_os_user}@${remote_cli_ip} "${db_dir}/sbin/start-cli.sh -u ${other_user} -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${other_user}@${remote_cli_ip} login"
   ssh ${os_name}@${db_ip} "${db_dir}/sbin/start-cli.sh -u ${other_user} -pw TimechoDB@2021 -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${other_user}@${db_ip} login"
   ...
   check_res "Authentication failed" 4 "${test_user} wrong password 4 times"
   echo "">${current_dir}/tmp.out
   for i in {1..3}
   do
      ssh ${remote_cli_os_user}@${remote_cli_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw wrongpassword -e \"show cluster;\"">>${current_dir}/tmp.out
...
# unlock user
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -pw TimechoDB@2021 -sql_dialect table -e "alter user ${test_user} account unlock;">${current_dir}/tmp.out
   check_res "success" 1 "unlock global ${test_user}"
   # right password
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${test_user} after unlock login"
# unlock santos
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -pw TimechoDB@2021 -sql_dialect table -e "alter user ${other_user} account unlock;">${current_dir}/tmp.out
   check_res "success" 1 "unlock not-locked ${other_user}"
   # right password
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${test_user} after unlock login"
   ssh ${remote_cli_os_user}@${remote_cli_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${test_user} after unlock login at ${remote_cli_ip}"
   ssh ${os_name}@${db_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -pw TimechoDB@2021 -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${test_user} after unlock login at ${db_ip}"

   # right password
   ${cli_dir}/sbin/start-cli.sh -u ${other_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "Running" ${node_num} "not-locked ${other_user} after unlock login"
   ssh ${remote_cli_os_user}@${remote_cli_ip} "${db_dir}/sbin/start-cli.sh -u ${other_user} -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "not-locked ${other_user} after unlock login"
   ssh ${os_name}@${db_ip} "${db_dir}/sbin/start-cli.sh -u ${other_user} -pw TimechoDB@2021 -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "not-locked ${other_user} at ${db_ip} after unlock login"
...
function testcase5()
{
   ...
   for i in {1..5}
   do
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -pw TimechoDB -e "show cluster;">>${current_dir}/tmp.out
   done
   check_res "Authentication failed" 5 "${db_admin_name} 5 times wrong password"
   echo "">${current_dir}/tmp.out
# this client ip right password
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${db_admin_name} at this shell ip is locked"
   
   # db_ip right password
   ssh ${os_name}@${db_ip} "${db_dir}/sbin/start-cli.sh -u ${db_admin_name} -pw TimechoDB@2021 -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${db_admin_name}@${db_ip} is not locked"
# this client ip right password
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${db_admin_name} at this shell ip is still locked"
# root unlock himself
   ssh ${os_name}@${db_ip} "${db_dir}/sbin/start-cli.sh -u ${db_admin_name} -pw TimechoDB@2021 -h ${query_ip} -sql_dialect tree -e \"alter user ${db_admin_name} @ '${this_shell_ip}' account unlock;\"">${current_dir}/tmp.out
   check_res "success" 1 "${db_admin_name}@${db_ip} exec unlock ${db_admin_name} @ '${this_shell_ip}'"
# this client ip right password
   ${cli_dir}/sbin/start-cli.sh -u ${db_admin_name} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "Running" ${node_num} "after unlock ${db_admin_name} at this shell ip login"

   check_log "testcase5"
   if [[ ${fail_flag} -gt 0 ]];then
      let sum_fail_flag++
      v_warnMessage="${v_warnMessage}testcase5 failed."
   fi
