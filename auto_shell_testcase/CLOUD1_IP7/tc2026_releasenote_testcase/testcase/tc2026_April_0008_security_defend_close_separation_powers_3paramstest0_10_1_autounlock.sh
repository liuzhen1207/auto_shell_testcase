...
configure_confignode() {
    ...
    ssh -o ConnectTimeout=10 "${os_user_name}@${node_ip}" bash -s <<EOF >> "${log_file}" 2>&1
        ...
        batch_set_sys_conf ".*failed_login_attempts=.*" "failed_login_attempts=0"
...
function testcase1()
{
   ...
   check_res "Authentication failed" 10 "${test_user} at shell client ip 10 times wrong password"
   echo "">${current_dir}/tmp.out
# this client ip right password
   ...
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${test_user} at shell client been locked ip right password"
   ...
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${test_user}@${db_ip} right password been locked"
# this client ip right password
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${test_user} at shell client been locked ip right password"
...
# this client ip right password,auto unlocked
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${test_user} at shell client after 60s auto unlocked login"

   # db_ip right password
   ssh ${os_name}@${db_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -pw TimechoDB@2021 -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${test_user}@${db_ip} after 60s auto unlocked login"
# remote client ip right password
   ...
   check_res "Running" ${node_num} "${test_user}@${remote_cli_ip} after 60s auto unlocked login"
