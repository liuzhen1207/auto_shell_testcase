...
# failed_login_attempts_per_user=1
# password_lock_time_minutes=10
# failed_login_attempts_per_user < failed_login_attempts ; global less than per ip
...
function testcase1()
{
   ...
   check_res "Authentication failed" 1 "${test_user} 1 wrong password"
   ...
   check_res "822: Account is blocked due to consecutive failed logins" 2 "${test_user} 2 right password"
  ...
  # right pass remote cli ip failed 4
   ...
   check_res "822: Account is blocked due to consecutive failed logins" 4 "${test_user} remote client 4 right password"
   echo "">${current_dir}/tmp.out
# unlock user ip
   ...
   check_res "success" 1 "alter user ${test_user} @ ${this_shell_ip} account unlock expect invalid"
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB -e "show cluster;">>${current_dir}/tmp.out
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${test_user} after unlock still lock explect login failed"
# unlock user
   ...
   check_res "Running" ${node_num} "${test_user} after unlock login exp succ"

   # db_ip right password
   ssh ${os_name}@${db_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -pw TimechoDB@2021 -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${test_user} at db ip after unlock login exp succ"
# remote client ip right password
   ssh ${remote_cli_os_user}@${remote_cli_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -pw TimechoDB@2021 -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${test_user} at remote client after unlock login exp succ"
