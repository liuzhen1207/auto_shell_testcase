...
do
   ...
   v_err_11=`ssh ${os_user_name}@${line} "grep \"IP-level attempts=5, User-level attempts=1000, Lock time=10 minutes\" ${db_dir}/logs/*datanode*all*|wc -l"`
   v_dn_total_err=$((v_err+v_err2+v_err3+v_err4+v_err5+v_err6+v_err7+v_err8+v_err9+v_err10+v_err_11))
   if [[ ${v_npe} -gt 0 ]];then
#           let fail_flag++
#	   let backup_log_flag++
...
function testcase1()
{
   ...
   check_res "Authentication failed" 10 "${test_user} at client ip input 10 times wrong password"
   # dn ip failed 4
   echo "">${current_dir}/tmp.out
   for i in {1..10}
   do
   ...
   check_res "Authentication failed" 10 "${test_user}@${db_ip} db ip input 10 times wrong password"
  ...
  # remote cli ip failed 4
   ...
   check_res "Authentication failed" 10 "${test_user}@${remote_cli_ip} remote client ip input 10 times wrong password"
   echo "">${current_dir}/tmp.out
  # is not locked
   ...
   check_res "Running" ${node_num} "${test_user} client ip not locked"
   ...
   check_res "Running" ${node_num} "${test_user} db ip not locked"
# remote client ip right password
   ...
   check_res "Running" ${node_num} "${test_user} remote client ip not locked"
