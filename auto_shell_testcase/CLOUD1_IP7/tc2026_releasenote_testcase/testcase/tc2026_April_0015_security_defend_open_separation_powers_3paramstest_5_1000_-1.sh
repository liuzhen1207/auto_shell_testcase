...
configure_confignode() {
    ...
    ssh -o ConnectTimeout=10 "${os_user_name}@${node_ip}" bash -s <<EOF >> "${log_file}" 2>&1
        ...
        batch_set_sys_conf ".*failed_login_attempts_per_user=.*" "failed_login_attempts_per_user=1000"
...
check_res "822: Account is blocked due to consecutive failed logins" 1 "${test_user} at shell ip login failed because locked"
   sleep 65
# user ip lock   
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "822: Account is blocked due to consecutive failed logins" 1 "${test_user} after 65s right password login failed because password_lock_time_minutes=-1"
   ...
   check_res "Running" ${node_num} "${test_user} at db ip login succ because shell-client-ip locked" 
  ...
  # right pass remote cli ip failed 4
   ...
   check_res "Running" ${node_num} "${test_user} at remote client ip login succ because shell-client-ip locked" 
   echo "">${current_dir}/tmp.out
...
# unlock user ip
   ...
   ${cli_dir}/sbin/start-cli.sh -u ${test_user} -h ${query_ip} -pw TimechoDB@2021 -e "show cluster;">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${test_user} at shell client login succ because unlocked"

   # db_ip right password
   ssh ${os_name}@${db_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -pw TimechoDB@2021 -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${test_user} at db ip login succ "
# remote client ip right password
   ssh ${remote_cli_os_user}@${remote_cli_ip} "${db_dir}/sbin/start-cli.sh -u ${test_user} -pw TimechoDB@2021 -h ${query_ip}  -e \"show cluster;\"">${current_dir}/tmp.out
   check_res "Running" ${node_num} "${test_user} at remote client ip login succ" 
# drop user
