...
function testcase1()
{
   ...
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} ${ssl_str} -e "drop user pitt;create user pitt 'TimechoDB@2021';">${current_dir}/tmp.out
   check_res "success" 1 "repeate_testcase1"
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u root -pw TimechoDB@2021 -sql_dialect table ${ssl_str} -e "grant system to user pitt;">${current_dir}/tmp.out
   check_res "success" 1 "repeate_testcase1"
...
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u pitt -pw TimechoDB@2022 ${ssl_str} -e "show cluster;" >${current_dir}/tmp.out
...
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u root -pw TimechoDB@2021 ${ssl_str} -e "drop user pitt;" >${current_dir}/tmp.out
check_res "success" 1 "repeate_testcase2_3_4_5"
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u root -pw TimechoDB@2021 ${ssl_str} -e "create user pitt 'TimechoDB@2021';" >${current_dir}/tmp.out
check_res "success" 1 "repeate_testcase2_3_4_5"
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u root -pw TimechoDB@2021 -sql_dialect table ${ssl_str} -e "grant system to user pitt;">${current_dir}/tmp.out
check_res "success" 1 "repeate_testcase2_3_4_5"
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u pitt -pw TimechoDB@2021 ${ssl_str} -e "show cluster;" >${current_dir}/tmp.out
...
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u root -pw TimechoDB@2021 -sql_dialect table ${ssl_str} -e "grant system to user pitt;"
check_res "success" 1 "repeate_testcase6"
# os +1
if [[ ${user_name} = root ]];then
ssh ${user_name}@${db_ip} "date -s \"+1 day\""
...
#     ssh ${user_name}@${db_ip} "sed -i 's/password_stale_warning_days=.*/password_stale_warning_days=-1/g' ${db_dir}/conf/iotdb-system.properties"
#   fi
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u root -pw TimechoDB@2021 -sql_dialect table  ${ssl_str} -e "set configuration password_stale_warning_days='-1';" >${current_dir}/tmp.out
check_res "Your password has not been changed for over 1 days" 1 "repeate_testcase6"
check_res "success" 1 "repeate_testcase6"
# restart_dn_cn
stop_dn_cn
...
start_dn_cn_close_separation
# drop user,create user
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u root -pw TimechoDB@2021 ${ssl_str} -e "drop user pitt;"
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u root -pw TimechoDB@2021 ${ssl_str} -e "create user pitt 'TimechoDB@2021';" >${current_dir}/tmp.out
...
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u root -pw TimechoDB@2021 ${ssl_str} -e "alter user pitt rename to santos;"
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u santos -pw TimechoDB@2021 ${ssl_str} -e "show cluster;" >${current_dir}/tmp.out
check_res "Your password has not been changed for over 30 days" 1 "testcase9"

# drop test user
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u root -pw TimechoDB@2021 ${ssl_str} -e "drop user pitt;" >${current_dir}/tmp.out
check_res "success" 0 "repeate"
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u root -pw TimechoDB@2021 ${ssl_str} -e "drop user santos;" >${current_dir}/tmp.out
check_res "success" 1 "repeate"
