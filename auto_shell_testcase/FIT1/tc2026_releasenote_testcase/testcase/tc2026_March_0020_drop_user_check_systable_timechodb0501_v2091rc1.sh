...
function testcase1()
{
    ...
    # check root.__audit
    ${cli_dir}/sbin/start-cli.sh -h "${query_ip}" -e "show devices root.__audit.password_history.*;" >${cur_dir}/tmp.out
    check_res "root.__audit.password_history._0" 1 
    cat ${cur_dir}/tmp.out

    # root 创建用户 + 授权
    ${cli_dir}/sbin/start-cli.sh -h "${query_ip}" -e "create user lily 'TimechoDB@2021';">${cur_dir}/tmp.out
    check_res "success" 1
    ${cli_dir}/sbin/start-cli.sh -h "${query_ip}" -e "create user kevin 'TimechoDB@2021';">${cur_dir}/tmp.out
    check_res "success" 1
    # get userid
    ${cli_dir}/sbin/start-cli.sh -h "${query_ip}" -e "list user;">${cur_dir}/tmp.out
    ...
    # drop user
    ${cli_dir}/sbin/start-cli.sh -h "${query_ip}" -e "drop user lily;">${cur_dir}/tmp.out
    check_res "success" 1
    ${cli_dir}/sbin/start-cli.sh -h "${query_ip}" -e "drop user kevin;">${cur_dir}/tmp.out
    check_res "success" 1
    # check
    ${cli_dir}/sbin/start-cli.sh -h "${query_ip}" -e "show devices root.__audit.password_history.*;" >${cur_dir}/tmp.out
    cat ${cur_dir}/tmp.out
    ...
    check_res "root.__audit.password_history._${v_kevin_id}" 0
    ${cli_dir}/sbin/start-cli.sh -h "${query_ip}" -e "show devices root.__audit.password_history.*;" >${cur_dir}/tmp.out
    check_res "root.__audit.password_history" 1
