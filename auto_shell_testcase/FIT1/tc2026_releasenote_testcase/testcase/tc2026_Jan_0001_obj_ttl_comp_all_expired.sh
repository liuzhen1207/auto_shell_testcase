...
# testcase3 delete 1 row in memtable
function testcase3()
{
	echo "testcase3 at `date "+%Y-%m-%d %H:%M:%S"`"
	fail_flag=0
#create table
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "create database IF NOT EXISTS db1;"
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "DROP TABLE IF EXISTS db1.t_object;"
...
#${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e <${cur_dir}/${SQL_FILE}
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "${v_sql_ins_100}"
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "select * from db1.t_object;" > testcase3_bef_del_query.out
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "select * from db1.t_object offset 49 limit 1;"|grep "+08:00" >${cur_dir}/tmp.out
v_del_time_value=`cat ${cur_dir}/tmp.out|awk -F "|" '{gsub(" ","");print $2}'`
v_dev_id=`cat ${cur_dir}/tmp.out|awk -F "|" '{gsub(" ","");print $3}'`
v_color=`cat ${cur_dir}/tmp.out|awk -F "|" '{gsub(" ","");print $4}'`
v_age=`cat ${cur_dir}/tmp.out|awk -F "|" '{gsub(" ","");print $5}'`
v_dep=`cat ${cur_dir}/tmp.out|awk -F "|" '{gsub(" ","");print $6}'`
echo "v_del_time_value=${v_del_time_value}"
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "delete from db1.t_object where time=${v_del_time_value} and device_id='${v_dev_id}' and color='${v_color}' and age='${v_age}' and department='${v_dep}';" 
#get bin_tmp count
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "flush;"
sleep 2
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "select * from db1.t_object;" > testcase3_aft_del_query.out
bef_ttl_obj_total_count=0
exec 3<${nodeinfo_dir}/datanode.txt
while read line<&3
do
...
# 最终状态输出
if [ ${fail_flag} -eq 0 ]; then
    echo "${SCRIPT_NAME} testcase3 pass." 
else
    echo "${SCRIPT_NAME} testcase3 fail." 
fi

}
# testcase4 delete 1 row after flush 
function testcase4()
{
	echo "testcase4 at `date "+%Y-%m-%d %H:%M:%S"`"
	fail_flag=0
#create table
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "create database IF NOT EXISTS db1;"
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "DROP TABLE IF EXISTS db1.t_object;"
...
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "${v_sql_ins_100}"
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "flush;"
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "select * from db1.t_object;" > testcase4_bef_del_query.out
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "select * from db1.t_object offset 49 limit 1;"|grep "+08:00" >${cur_dir}/tmp.out
v_del_time_value=`cat ${cur_dir}/tmp.out|awk -F "|" '{gsub(" ","");print $2}'`
v_dev_id=`cat ${cur_dir}/tmp.out|awk -F "|" '{gsub(" ","");print $3}'`
v_color=`cat ${cur_dir}/tmp.out|awk -F "|" '{gsub(" ","");print $4}'`
...
#get bin_tmp count
sleep 2
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "select * from db1.t_object;" > testcase4_aft_del_query.out
bef_ttl_obj_total_count=0
exec 3<${nodeinfo_dir}/datanode.txt
while read line<&3
do
...
# 最终状态输出
if [ ${fail_flag} -eq 0 ]; then
    echo "${SCRIPT_NAME} testcase4 pass." 
else
    echo "${SCRIPT_NAME} testcase4 fail." 
fi
