...
configure_confignode() {
    ...
    ssh -o ConnectTimeout=10 "${os_user_name}@${node_ip}" bash -s <<EOF >> "${log_file}" 2>&1
        ...
        batch_set_sys_conf ".*schema_replication_factor=.*" "schema_replication_factor=1"
        batch_set_sys_conf ".*data_replication_factor=.*" "data_replication_factor=1"
        batch_set_sys_conf ".*query_timeout_threshold=.*" "query_timeout_threshold=60000000"
        batch_set_sys_conf ".*dn_thrift_max_frame_size=.*" "dn_thrift_max_frame_size=1610612736"
        batch_set_sys_conf ".*degree_of_query_parallelism=.*" "degree_of_query_parallelism=1"
        batch_set_sys_conf ".*datanode_memory_proportion=.*" "datanode_memory_proportion=3:6:1:1:1:1"
...
sed -i "s/^PASSWORD=.*/PASSWORD=${bm_root_pw}/g" ${bm_dir}/${bm_conf}/config.properties
...
${bm_dir}/benchmark.sh -cf ${bm_dir}/${bm_conf} >${bm_log_dir}/${t}_bm.out
v_check_okpoint=`grep INGESTION ${bm_log_dir}/${t}_bm.out|grep 100000|grep 30000000|wc -l`
if [[ ${v_check_okpoint} = 0 ]];then
let fail_flag++
echo "benchmark test result is not expected."
fi
}
# testcase1
function testcase1()
{
   ...
   v_sql="explain analyze verbose select last_value(s_1),max_value(s_2),min_value(s_3) into root.eioms_cm.g_0.::(value_last,value_max,value_min) from root.eioms_cm1.**  where time>= 1640966400000 & time< 1640966700000 group by TIME([1640966400000,1640966700000), 5m) align by device;"
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 36000 -e "${v_sql}" >${cur_dir}/tmp1.out 
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 36000 -e "${v_sql}" >${cur_dir}/tmp2.out
   v_cost_t1=`cat ${cur_dir}/tmp1.out |grep "It costs" |awk -F "It costs" '{print $2}'|awk -F 's' '{gsub(" ","");print $1}'|awk -F '.' '{print $1}'`
   v_cost_t2=`cat ${cur_dir}/tmp2.out |grep "It costs" |awk -F "It costs" '{print $2}'|awk -F 's' '{gsub(" ","");print $1}'|awk -F '.' '{print $1}'`
   if [[ ${v_cost_t1} -gt 35 ]] || [[ ${v_cost_t2} -gt 30 ]];then
	   let fail_flag++
	   echo "${v_cost_t1},${v_cost_t2}"
	   echo "testcase1 fail"
   else
	   echo "testcase1 pass"

   fi
   if [[ ${v_cost_t1} -lt ${v_cost_t2} ]];then
           let v_warnNum++
           v_warnMessage="first cost ${v_cost_t1} less than second cost ${v_cost_t2}."

   fi

}
# restart cluster ,clear cache ,query
function testcase2()
{
   ...
   if [ $? -eq 0 ]; then
       ...
       exit 1
        ...
        echo "testcase2 at `date "+%Y-%m-%d %H:%M:%S"`"
   v_sql="explain analyze verbose select last_value(s_1),max_value(s_2),min_value(s_3) into root.eioms_cm.g_0.::(value_last,value_max,value_min) from root.eioms_cm1.**  where time>= 1640966400000 & time< 1640966700000 group by TIME([1640966400000,1640966700000), 5m) align by device;"
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 36000 -e "${v_sql}" >${cur_dir}/tmp3.out
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 36000 -e "${v_sql}" >${cur_dir}/tmp4.out
   v_cost_t1=`cat ${cur_dir}/tmp3.out |grep "It costs" |awk -F "It costs" '{print $2}'|awk -F 's' '{gsub(" ","");print $1}'|awk -F '.' '{print $1}'`
   v_cost_t2=`cat ${cur_dir}/tmp4.out |grep "It costs" |awk -F "It costs" '{print $2}'|awk -F 's' '{gsub(" ","");print $1}'|awk -F '.' '{print $1}'`
   if [[ ${v_cost_t1} -gt 35 ]] || [[ ${v_cost_t2} -gt 30 ]];then
           let fail_flag++
           echo "${v_cost_t1},${v_cost_t2}"
           echo "testcase2 fail"
