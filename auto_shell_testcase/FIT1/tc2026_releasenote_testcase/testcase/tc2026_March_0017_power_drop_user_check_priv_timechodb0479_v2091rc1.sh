...
function testcase1()
{
    ...
    # root 创建用户 + 授权
    ${cli_dir}/sbin/start-cli.sh -u ${v_sec_super_user} -h "${query_ip}" -e "create user ${test_user} '${test_pwd_old}';">${cur_dir}/tmp.out
    check_res "success" 1
    ${cli_dir}/sbin/start-cli.sh -u ${v_sys_super_user} -h "${query_ip}" -sql_dialect table -e "grant system to user ${test_user};">${cur_dir}/tmp.out
    check_res "success" 1

    # ======================
    # 步骤0：记录第一次创建的 UserId
    # ======================
    ${cli_dir}/sbin/start-cli.sh -u "${v_sec_super_user}" -h "${query_ip}" -e "LIST USER;" > "${cur_dir}/user_id.tmp"
    ...
    # 步骤2：root 删除用户
    # ======================
    ${cli_dir}/sbin/start-cli.sh -u "${v_sys_super_user}" -h "${query_ip}" -sql_dialect table -e "revoke system from  user ${test_user};"
    ${cli_dir}/sbin/start-cli.sh -u "${v_sec_super_user}" -h "${query_ip}" -sql_dialect table -e "DROP USER ${test_user};"
    sleep 2
    ...
    # ======================
    ${cli_dir}/sbin/start-cli.sh -u "${v_sec_super_user}" -h "${query_ip}" -e "CREATE USER ${test_user} '${test_pwd_new}';"
    ${cli_dir}/sbin/start-cli.sh -u "${v_sys_super_user}" -h "${query_ip}" -sql_dialect table -e "grant ${test_priv} to user ${test_user};"
    sleep 1
    ...
    ${cli_dir}/sbin/start-cli.sh -u "${v_sys_super_user}" -h "${query_ip}" -sql_dialect table -e "revoke system from  user ${test_user};"
    ${cli_dir}/sbin/start-cli.sh -u "${v_sec_super_user}" -h "${query_ip}" -e "DROP USER ${test_user};"
