...
backup_dir=/data2/tc2026_data_backup/v2061_rc7_0915_802dd96_ns_ttl_iotv2
# 清理旧节点文件，复制新配置
...
do
   ...
   v_err11=`ssh ${os_user_name}@${line} "grep \"because PipePlugin PIPE-CONSENSUS-ASYNC-CONNECTOR has not been registered\" ${db_dir}/logs/*datanode*all*|grep \"WARN  o.a.i\" |wc -l"`
   v_err12=`ssh ${os_user_name}@${line} "grep \"The assigner queue content has exceeded half, it may be stuck and may block insertion\" ${db_dir}/logs/*datanode*all*|grep \"WARN  o.a.i\" |wc -l"`
   v_dn_total_err=$((v_err+v_err2+v_err3+v_err4+v_err5+v_err6+v_err7+v_err8+v_err9+v_err10+v_err11+v_err12))
...
#       let fail_flag++
#   fi
#echo "第二次,检查副本数据一致性(重启集群)"
#check_data_consistent
sleep 1080
echo "sleep 18min"
#echo "第三次,sleep 18min ,检查副本数据一致性"
echo "第二次,sleep 18min ,检查副本数据一致性"
check_data_consistent
#sh -x "${clean_env_dir}/stop_cluster.sh" >> "${log_file}" 2>&1
#   sh -x "${prepare_env_dir}/start_cluster_v20.sh" "3" "${total_node_num}" >> "${log_file}" 2>&1
#   if [ $? -eq 0 ]; then
#       log "INFO" "集群启动成功"
#   else
#       log "ERROR" "集群启动失败"
