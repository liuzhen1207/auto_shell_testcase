...
backup_dir=/data2/tc2026_data_backup/v2082_rc7_20250323_9a35b7e_object
# 清理旧节点文件，复制新配置
...
wait_sync_done 180
   ...
   sql1="select file_content,device_id,color,file_type,department,age from root.db1.t_object order by time,device_id,color,age;"
   sql2="select file_content,device_id,color,file_type,department,age from db1.t_object order by time,device_id,color,age;"
   # all online
   exception_num=0
   while true
   do
   ...
   if [[ ${exception_num} -ge 10 ]];then
	   let fail_flag++
	   v_warnMessage="${v_warnMessage}tree all online query 10 times failed."
	   break
   fi
   done
   v_diff_exp_act=$(diff ${cur_dir}/q_all_online_tree.out ${cur_dir}/exp_result/20260416_v2082rc7_data_q_tree.out|grep "08:00"|wc -l)
   if [[ ${v_diff_exp_act} -gt 0 ]];then
	   let fail_flag++
	   v_warnMessage="${v_warnMessage}tree all online query result is wrong."
   fi 
   exception_num=0
   while true
   do
   ...
   v_exception_num=$v_exception_num1
   if [[ ${v_exception_num} = 0 ]];then
           break
   else
	   let exception_num++
   ...
   if [[ ${exception_num} -ge 10 ]];then
           let fail_flag++
	   v_warnMessage="${v_warnMessage}table all online query 10 times failed."
           break
   fi
   done
   v_diff_exp_act=$(diff ${cur_dir}/q_all_online_table1.out ${cur_dir}/exp_result/20260416_v2082rc7_data_q_table.out|grep "08:00"|wc -l)
   if [[ ${v_diff_exp_act} -gt 0 ]];then
           let fail_flag++
           v_warnMessage="${v_warnMessage}table all online query result is wrong."
   fi
   ...
   query_ip2=`tail -1 ${cur_dir}/tmp_node.out`
      ...
      if [[ ${exception_num} -ge 10 ]];then
	      let fail_flag++
           v_warnMessage="${v_warnMessage}stop ip${v_ip} tree query failed."
	      break
      fi
      done
   ...
   v_exception_num=$v_exception_num1
      ...
      if [[ ${exception_num} -ge 10 ]];then
           v_warnMessage="${v_warnMessage}stop ip${v_ip} table query failed."
	      let fail_flag++
	      break
      fi
      done

      v_diff_tree=`diff ${cur_dir}/q_all_online_tree.out ${cur_dir}/q_stop_ip${v_ip}_tree.out|grep ""08:00""|wc -l`
      v_diff_table1=`diff ${cur_dir}/q_all_online_table1.out ${cur_dir}/q_stop_ip${v_ip}_table1.out|egrep ""08:00""|wc -l`
      v_diff_total=$((v_diff_tree+v_diff_table1))
...
do
   ...
   v_cn_err4=`ssh ${os_user_name}@${line} "grep \"Error waiting for roll back the COLLECT_EARLIEST_TIMESLOTS state due to thread interruption\" ${db_dir}/logs/*confignode*all*|wc -l"`
   if [[ ${v_npe} -gt 0 ]];then
	   let fail_flag++
	   let backup_log_flag++
   ...
   v_cn_err_total=$((v_cn_err1+v_cn_err2+v_cn_err3+v_cn_err4))
