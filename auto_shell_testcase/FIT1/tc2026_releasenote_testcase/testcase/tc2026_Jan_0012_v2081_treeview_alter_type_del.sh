...
function del_time_condition()
{
while true
do
   while read -r v_ts_row; do	
	   v_ts_path=`echo ${v_ts_row} |awk -F '|' '{print $2}'`
	   v_ts_name=`echo ${v_ts_path} |awk -F '.' '{print $5}'`
	   v_ts_pre_path=`echo ${v_ts_path} |awk -F '.' '{print $1"."$2"."$3"."$4}'`

           local v_max_time=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect tree -timeout 3600 -e "select ${v_ts_name} from ${v_ts_pre_path} order by time desc limit 1;"|grep "+08:00"|awk -F '|' '{print $2}'`
                    # delete
           if [[ "${v_max_time}" != "" ]];then
                 ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect tree -timeout 3600 -e "delete from ${v_ts_path} where time<=${v_max_time};"
           fi
...
while true
do
	v_ok_num=`cat ${cur_dir}/show_ts.txt |wc -l`
	if [[ ${v_ok_num} -ge 100 ]];then
		break
	else 
		sleep 5
	fi
