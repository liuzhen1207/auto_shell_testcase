...
configure_confignode() {
    ...
    ssh -o ConnectTimeout=10 "${os_user_name}@${node_ip}" bash -s <<EOF >> "${log_file}" 2>&1
        ...
        batch_set_sys_conf ".*ttl_check_interval=.*" "ttl_check_interval=120000"
...
#check log
${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${ssl_str} -h ${query_ip} -e "set ttl to root.test.g_0.d1_4 604800000;"
${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${ssl_str} -h ${query_ip} -e "set ttl to root.test.g_0.d2_4 604800000;"
check_log
