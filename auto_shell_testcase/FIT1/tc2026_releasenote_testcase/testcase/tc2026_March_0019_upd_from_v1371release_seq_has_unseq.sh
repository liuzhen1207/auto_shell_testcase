...
configure_confignode() {
    ...
    ssh -o ConnectTimeout=10 "${os_user_name}@${node_ip}" bash -s <<EOF >> "${log_file}" 2>&1
        ...
        batch_set_sys_conf ".*enable_auto_repair_compaction=.*" "enable_auto_repair_compaction=true"
...
#       exit 1
  ...
  # copy backup data
 backup_dir=/data2/tc2026_data_backup/v1371_release
 exec 3<${nodeinfo_dir}/datanode.txt
 while read line<&3
 do
	 ssh ${os_user_name}@${line} "cp -rp ${backup_dir}/data1_backup ${data1_dir}"
	 ssh ${os_user_name}@${line} "cp -rp ${backup_dir}/data2_backup ${db_dir}/data"
	 ssh ${os_user_name}@${line} "cp -rp ${backup_dir}/data3_backup ${data3_dir}"
	 sleep 1
 done
 exec 3<${nodeinfo_dir}/confignode.txt
 while read line<&3
 do
         cp -rp ${backup_dir}/data_backup ${db_dir}/data
         sleep 1
 done
...
wait_sync_done 180
   ...
   sql2="select count(s_0),count(s_1),count(s_2),count(s_3),count(s_4),count(s_5),count(s_6),count(s_7),count(s_8),count(s_9) from root.tmp1.g_0.** align by device;"
   sql3="select count(s_0),count(s_1),count(s_2),count(s_3),count(s_4),count(s_5),count(s_6),count(s_7),count(s_8),count(s_9) from root.tmp2.g_0.** align by device;"
   # all online
   exception_num=0
   while true
   do
   ...
   ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${ssl_str} -h ${query_ip} -sql_dialect tree -timeout 36000 -e "${sql2}" >${cur_dir}/q_all_online_tmp1.out
   ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${ssl_str} -h ${query_ip} -sql_dialect tree -timeout 36000 -e "${sql3}" >${cur_dir}/q_all_online_tmp2.out
   v_exception_num1=`grep Exception ${cur_dir}/q_all_online_tree.out|wc -l`
   v_exp_num1=`grep root.test ${cur_dir}/q_all_online_tree.out|wc -l`
   v_exp_num2=`grep root.tmp1 ${cur_dir}/q_all_online_tmp1.out|wc -l`
   v_exp_num3=`grep root.tmp2 ${cur_dir}/q_all_online_tmp2.out|wc -l`
   v_exception_num2=`grep Exception ${cur_dir}/q_all_online_tmp1.out|wc -l`
   v_exception_num3=`grep Exception ${cur_dir}/q_all_online_tmp2.out|wc -l`
   v_exception_num=$((v_exception_num1+v_exception_num2+v_exception_num3))
   ...
   if [[ ${v_exp_num1} != 20 ]] || [[ ${v_exp_num2} != 10 ]] || [[ ${v_exp_num3} != 10 ]];then
	   let fail_flag++
	 v_warnMessage="${v_warnMessage}query result is unexpected."
   ...
   query_ip2=`tail -1 ${cur_dir}/tmp.out`
      ...
      ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${ssl_str} -h ${query_ip} -sql_dialect tree -timeout 36000 -e "${sql2}" >${cur_dir}/q_stop_ip${v_ip}_tmp1.out
      ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${ssl_str} -h ${query_ip} -sql_dialect tree -timeout 36000 -e "${sql3}" >${cur_dir}/q_stop_ip${v_ip}_tmp2.out
      v_exception_num1=`grep Exception ${cur_dir}/q_stop_ip${v_ip}_tree.out|wc -l`
      v_exception_num2=`grep Exception ${cur_dir}/q_stop_ip${v_ip}_tmp1.out|wc -l`
      v_exception_num3=`grep Exception ${cur_dir}/q_stop_ip${v_ip}_tmp2.out|wc -l`
      v_exception_num=$((v_exception_num1+v_exception_num2+v_exception_num3))
      if [[ ${v_exception_num} = 0 ]];then
           break
      else
      ...
      v_diff_tree1=`diff ${cur_dir}/q_all_online_tree.out ${cur_dir}/q_stop_ip${v_ip}_tree.out|grep "root"|wc -l`
      v_diff_tree2=`diff ${cur_dir}/q_all_online_tmp1.out ${cur_dir}/q_stop_ip${v_ip}_tmp1.out|grep "root"|wc -l`
      v_diff_tree3=`diff ${cur_dir}/q_all_online_tmp2.out ${cur_dir}/q_stop_ip${v_ip}_tmp2.out|grep "root"|wc -l`
      v_diff_tree=$((v_diff_tree1+v_diff_tree2+v_diff_tree3))
      if [[ ${v_diff_tree} -gt 0 ]];then
         let fail_flag++
	 let backup_data_log_flag++
	 diff ${cur_dir}/q_all_online_tree.out ${cur_dir}/q_stop_ip${v_ip}_tree.out
	 diff ${cur_dir}/q_all_online_tmp1.out ${cur_dir}/q_stop_ip${v_ip}_tmp1.out
	 diff ${cur_dir}/q_all_online_tmp2.out ${cur_dir}/q_stop_ip${v_ip}_tmp2.out
	 v_warnMessage="${v_warnMessage}data inconsistent."
         echo "diff : ${v_diff_tree}"
      fi
   done
...
do
   ...
   v_err11=`ssh ${os_user_name}@${line} "grep \"REPAIR task meets error:\" ${db_dir}/logs/*datanode*all*|wc -l"`
   v_err12=`ssh ${os_user_name}@${line} "grep \"PlainDeviceTimeIndex.serialize:\" ${db_dir}/logs/*datanode*all*|grep \"Unknown Source\"|wc -l"`
   v_dn_total_err=$((v_err+v_err2+v_err4+v_err5+v_err6+v_err7+v_err8+v_err9+v_err10+v_err11+v_err12))
   ...
   if [[ ${v_err3} != 3 ]];then
	   let fail_flag++
	   let backup_log_flag++
	   v_warnMessage="${v_warnMessage}DN EXPECT 3 ERROR but get ${v_err3}."
   fi
done
