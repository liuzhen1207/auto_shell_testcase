...
function check_log_INTERNAL_SERVER_ERROR()
{
	exec 3<${nodeinfo_dir}/datanode.txt
while read line<&3
do
   ssh ${os_user_name}@${line} "sudo gunzip ${db_dir}/logs/*datanode*all*"
   v_npe=`ssh ${os_user_name}@${line} "grep INTERNAL_SERVER_ERROR ${db_dir}/logs/*datanode*all*|wc -l"`

   if [[ ${v_npe} -gt 0 ]];then
           let fail_flag++
           let backup_log_flag++
           v_warnMessage="${v_warnMessage}DN ${line} INTERNAL_SERVER_ERROR ${v_npe}."
           echo "DN ${line} INTERNAL_SERVER_ERROR : ${v_npe}"
   fi
...
wait_sync_done 180
   ...
   cat ${cur_dir}/tmp.out |grep Running|awk -F "|" '{gsub(" ","");print $4}'>${cur_dir}/tmp1.out
   mv ${cur_dir}/tmp1.out ${cur_dir}/tmp.out
   sql1="select device_id,color,file_type,department,age from root.db1.t_object order by time,device_id,color,age ;"
   sql2="show devices root.db1.**"
   sql3="select time,file_content,device_id,color,file_type,department,age from db1.t_object order by time,device_id,color,age ;"
   # all online
   exception_num=0
   while true
   do
   ...
   ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${ssl_str} -h ${query_ip} -sql_dialect tree -timeout 36000 -e "${sql2}" >${cur_dir}/q_all_online_tree2.out
   v_exception_num=`grep Exception ${cur_dir}/q_all_online_tree.out|wc -l`
   v_exception_num2=`grep Exception ${cur_dir}/q_all_online_tree2.out|wc -l`
   v_exp_row=$(grep 1970-04-15 ${cur_dir}/q_all_online_tree.out|wc -l)
   v_exp_row2=$(grep root.db1 ${cur_dir}/q_all_online_tree.out|wc -l)
   if [[ ${v_exception_num} = 0 ]] || ${v_exception_num2} = 0 ]];then
	   break
   else
           let exception_num++
	   cat ${cur_dir}/q_all_online_tree.out
	   cat ${cur_dir}/q_all_online_tree2.out
	   sleep 1
   fi
...
if [[ ${v_exp_row} = 0 ]] || ${v_exp_row2} = 0 ]];then
	let fail_flag++
        v_warnMessage="${v_warnMessage}tree query failed."

fi
      exception_num=0
   while true
   do
   ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${ssl_str} -h ${query_ip} -sql_dialect table -timeout 36000 -e "${sql3}" >${cur_dir}/q_all_online_table1.out
   v_exception_num=`grep Exception ${cur_dir}/q_all_online_table1.out|wc -l`
   v_exp_row=$(grep 1970-04-15 ${cur_dir}/q_all_online_table1.out|wc -l)
   ...
   query_ip2=`tail -1 ${cur_dir}/tmp.out`
      ...
      ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${ssl_str} -h ${query_ip} -sql_dialect tree -timeout 36000 -e "${sql2}" >${cur_dir}/q_stop_ip${v_ip}_tree2.out
      v_exception_num=`grep Exception ${cur_dir}/q_stop_ip${v_ip}_tree.out|wc -l`
   v_exception_num2=`grep Exception ${cur_dir}/q_stop_ip${v_ip}_tree2.out|wc -l`
   v_exp_row=$(grep 1970-04-15 ${cur_dir}/q_all_online_tree.out|wc -l)
   v_exp_row2=$(grep 1970-04-15 ${cur_dir}/q_all_online_tree2.out|wc -l)
   if [[ ${v_exception_num} = 0 ]] || ${v_exception_num2} = 0 ]];then
           break
   else
           let exception_num++
   ...
   if [[ ${exception_num} -ge 10 ]];then
      ...
      while true
      do
      ${cli_dir}/sbin/start-cli.sh -u ${db_user_name} ${ssl_str} -h ${query_ip} -sql_dialect table -timeout 36000 -e "${sql3}" >${cur_dir}/q_stop_ip${v_ip}_table1.out
      ...
      v_diff_tree2=`diff ${cur_dir}/q_all_online_tree2.out ${cur_dir}/q_stop_ip${v_ip}_tree2.out|grep "root"|wc -l`
      v_diff_table1=`diff ${cur_dir}/q_all_online_table1.out ${cur_dir}/q_stop_ip${v_ip}_table1.out|grep "1970"|wc -l`
      v_diff_num=$((v_diff_tree+v_diff_tree2+v_diff_table1))
      if [[ ${v_diff_num} -gt 0 ]];then
         let fail_flag++
	 let backup_data_log_flag++
	 diff ${cur_dir}/q_all_online_tree.out ${cur_dir}/q_stop_ip${v_ip}_tree.out
	 diff ${cur_dir}/q_all_online_tree2.out ${cur_dir}/q_stop_ip${v_ip}_tree2.out
...
do
   ...
   v_err11=`ssh ${os_user_name}@${line} "grep \"Integer overflow when converting\" ${db_dir}/logs/*datanode*all*|wc -l"`
...
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect tree -e "CREATE ALIGNED TIMESERIES root.db1.t_object(
...
check_res success 1 "create aligned TIMESERIES"
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "INSERT INTO db1.t_object(time, device_id, color, age, department, file_type, file_content) VALUES
...
('1970-04-15T18:52:15.004+08:00', 'd5', 'black', '22', 'dev', 'image', to_object(true,0,X'0c9d8e7f6a5b4c3d'));INSERT INTO root.db1.t_object(time, device_id, color, age, department, file_type) VALUES
('1970-04-15T18:52:15.000+08:00', 'd1', 'red', '18', 'dev', 'image'),
('1970-04-15T18:52:15.001+08:00', 'd2', 'blue', '19', 'test', 'movie'),
('1970-04-15T18:52:15.002+08:00', 'd3', 'green', '20', 'product', 'json'),
('1970-04-15T18:52:15.003+08:00', 'd4', 'yellow', '21', 'ops', 'txt'),
('1970-04-15T18:52:15.004+08:00', 'd5', 'black', '22', 'dev', 'image')">${cur_dir}/tmp.out
check_res success 1 "tablemode:insert table and tree"
check_res Exception 1 "tablemode:insert table and tree"
cat ${cur_dir}/tmp.out
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect tree -e "INSERT INTO db1.t_object(time, device_id, color, age, department, file_type, file_content) VALUES
(1970-04-15T18:52:15.000+08:00, 'd1', 'red', '18', 'dev', 'image', to_object(true,0,X'5f8a7b9c4d2e8f1a')),
(1970-04-15T18:52:15.001+08:00, 'd2', 'blue', '19', 'test', 'movie', to_object(true,0,X'7e9b8c0d3f4a5b6c')),
(1970-04-15T18:52:15.002+08:00, 'd3', 'green', '20', 'product', 'json', to_object(true,0,X'8a7b6c5d4e3f2a1b')),
(1970-04-15T18:52:15.003+08:00, 'd4', 'yellow', '21', 'ops', 'txt', to_object(true,0,X'9b8c7d6e5f4a3b2c')),
...
(1970-04-15T18:52:15.003+08:00, 'd4', 'yellow', '21', 'ops', 'txt'),
(1970-04-15T18:52:15.004+08:00, 'd5', 'black', '22', 'dev', 'image')">${cur_dir}/tmp.out
check_res success 1 "treemode:insert table and tree"
check_res Exception 1 "treemode:insert table and tree"
cat ${cur_dir}/tmp.out
...
#check log
check_log
check_log_INTERNAL_SERVER_ERROR
