...
configure_confignode() {
    ...
    ssh -o ConnectTimeout=10 "${os_user_name}@${node_ip}" bash -s <<EOF >> "${log_file}" 2>&1
        ...
        batch_set_sys_conf ".*datanode_memory_proportion=.*" "datanode_memory_proportion=5:1:1:1:1:1"
        batch_set_sys_conf ".*target_chunk_point_num=.*" "target_chunk_point_num=1000000000"
        batch_set_sys_conf ".*target_chunk_size=.*" "target_chunk_size=160000000000"
#        batch_set_sys_conf ".*max_number_of_points_in_page=.*" "max_number_of_points_in_page=1000"
#        batch_set_sys_conf ".*flush_proportion=.*" "flush_proportion=0.8"
        batch_set_sys_conf ".*enable_timed_flush_seq_memtable=.*" "enable_timed_flush_seq_memtable=false"
        batch_set_sys_conf ".*enable_timed_flush_unseq_memtable=.*" "enable_timed_flush_unseq_memtable=false"
        batch_set_sys_conf ".*wal_min_effective_info_ratio=.*" "wal_min_effective_info_ratio=0.0001"
...
configure_datanode() {
    ...
    ssh -o ConnectTimeout=10 "${os_user_name}@${node_ip}" bash -s <<EOF >> "${log_file}" 2>&1
        ...
        batch_set_sys_conf ".*default_data_region_group_num_per_database=.*" "default_data_region_group_num_per_database=1"
        batch_set_sys_conf ".*datanode_memory_proportion=.*" "datanode_memory_proportion=5:1:1:1:1:1"
        batch_set_sys_conf ".*target_chunk_point_num=.*" "target_chunk_point_num=100000000"
        batch_set_sys_conf ".*target_chunk_size=.*" "target_chunk_size=16000000000"
#        batch_set_sys_conf ".*max_number_of_points_in_page=.*" "max_number_of_points_in_page=1000"
#        batch_set_sys_conf ".*flush_proportion=.*" "flush_proportion=0.8"
        batch_set_sys_conf ".*enable_timed_flush_seq_memtable=.*" "enable_timed_flush_seq_memtable=false"
        batch_set_sys_conf ".*enable_timed_flush_unseq_memtable=.*" "enable_timed_flush_unseq_memtable=false"
...
function start_bm()
{
bm_dir=/ssd/benchmark/bm_v13_sel_into_20260109
bm_conf=conf_count_ts
#get root password
...
sed -i "s/^PASSWORD=.*/PASSWORD=${bm_root_pw}/g" ${bm_dir}/${bm_conf}/conf1/config.properties
sed -i "s/^PASSWORD=.*/PASSWORD=${bm_root_pw}/g" ${bm_dir}/${bm_conf}/conf2/config.properties
...
${bm_dir}/benchmark.sh -cf ${bm_dir}/${bm_conf}/conf1 >${bm_log_dir}/${t}_bm1.out &
${bm_dir}/benchmark.sh -cf ${bm_dir}/${bm_conf}/conf2 >${bm_log_dir}/${t}_bm2.out
wait
}
function check_cost()
{
	check_res_file=$1
	check_exp_max_value=$2
	v_act_value=`cat ${check_res_file} |grep "It costs"|awk -F "It costs" '{print $2}'|awk -F 's' '{gsub(" ","");print $1}'`
       if (( $(echo "$v_act_value > $check_exp_max_value" | bc -l) )); then
          echo "$v_act_value 大于 $check_exp_max_value"
	  let fail_flag++
       else
          echo "$v_act_value 不大于 $check_exp_max_value"
       fi
 	
}
function query_ts_seq()
{
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 120 -e "count timeseries;">${cur_dir}/tmp.out
check_cost "${cur_dir}/tmp.out" 1
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 120 -e "count devices;">${cur_dir}/tmp.out
check_cost "${cur_dir}/tmp.out" 1
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 120 -e "count timeseries root.test.** where time>2020-01-01T00:00:00+08:00 and time<2021-01-04T00:00:00+08:00;" >${cur_dir}/tmp.out
check_cost "${cur_dir}/tmp.out" 1
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 120 -e "count devices root.test.** where time>2020-01-01T00:00:00+08:00 and time<2021-01-04T00:00:00+08:00;">${cur_dir}/tmp.out
check_cost "${cur_dir}/tmp.out" 1
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 120 -e "show timeseries root.test.** where time>2020-01-01T00:00:00+08:00 and time<2021-01-04T00:00:00+08:00 limit 1;">${cur_dir}/tmp.out
check_cost "${cur_dir}/tmp.out" 1
