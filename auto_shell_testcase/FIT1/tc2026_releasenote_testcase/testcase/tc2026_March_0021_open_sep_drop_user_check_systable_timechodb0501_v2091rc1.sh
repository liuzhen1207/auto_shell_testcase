...
#        batch_set_sys_conf ".*password_expiration_days=.*" "password_expiration_days=10"
#batch_set_sys_conf ".*enable_encrypt_config_file=.*" "enable_encrypt_config_file=false"
#batch_set_sys_conf ".*enable_encrypt_permission_file=.*" "enable_encrypt_permission_file=false"
#batch_set_sys_conf ".*enable_thrift_ssl=.*" "enable_thrift_ssl=false"
#batch_set_sys_conf ".*enable_internal_ssl=.*" "enable_internal_ssl=false"
#batch_set_sys_conf ".*password_reuse_interval_days=.*" "password_reuse_interval_days=2"
batch_set_sys_conf ".*enable_separation_of_powers=.*" "enable_separation_of_powers=true"
#batch_set_sys_conf ".*encrypt_type=.*" "encrypt_type=org.apache.tsfile.encrypt.UNENCRYPTED"
EOF
...
function testcase1()
{
    ...
    # check root.__audit
    v_audit_user="audit_admin"
    v_sec_user="security_admin"
    ${cli_dir}/sbin/start-cli.sh -u ${v_audit_user} -h "${query_ip}" -e "show devices root.__audit.password_history.*;" >${cur_dir}/tmp.out
    check_res "root.__audit.password_history._0" 0 
    cat ${cur_dir}/tmp.out

    # root 创建用户 + 授权
    ${cli_dir}/sbin/start-cli.sh -u ${v_sec_user} -h "${query_ip}" -e "create user lily 'TimechoDB@2021';">${cur_dir}/tmp.out
    check_res "success" 1
    ${cli_dir}/sbin/start-cli.sh -u ${v_sec_user} -h "${query_ip}" -e "create user kevin 'TimechoDB@2021';">${cur_dir}/tmp.out
    check_res "success" 1
    # get userid
    ${cli_dir}/sbin/start-cli.sh -u ${v_sec_user} -h "${query_ip}" -e "list user;">${cur_dir}/tmp.out
    v_lily_id=$(grep lily ${cur_dir}/tmp.out|awk -F '|' '{gsub(" ","");print $2}')
    v_kevin_id=$(grep lily ${cur_dir}/tmp.out|awk -F '|' '{gsub(" ","");print $2}')
    ${cli_dir}/sbin/start-cli.sh -h "${query_ip}" -u lily -e " LIST PRIVILEGES of user lily;">${cur_dir}/tmp.out
    check_res "Empty set" 1
    ${cli_dir}/sbin/start-cli.sh -h "${query_ip}" -u kevin -e " LIST PRIVILEGES of user kevin;">${cur_dir}/tmp.out
    check_res "Empty set" 1

    ${cli_dir}/sbin/start-cli.sh -u ${v_audit_user} -h "${query_ip}" -e "show devices root.__audit.password_history.*;" >${cur_dir}/tmp.out
    check_res "root.__audit.password_history._${v_lily_id}" 1 
    check_res "root.__audit.password_history._${v_kevin_id}" 1 
    # drop user
    ${cli_dir}/sbin/start-cli.sh -u ${v_sec_user} -h "${query_ip}" -e "drop user lily;">${cur_dir}/tmp.out
    check_res "success" 1
    ${cli_dir}/sbin/start-cli.sh -u ${v_sec_user} -h "${query_ip}" -e "drop user kevin;">${cur_dir}/tmp.out
    check_res "success" 1
    # check
    ${cli_dir}/sbin/start-cli.sh -u ${v_audit_user} -h "${query_ip}" -e "show devices root.__audit.password_history.*;" >${cur_dir}/tmp.out
    cat ${cur_dir}/tmp.out
    check_res "root.__audit.password_history._${v_lily_id}" 0 
    check_res "root.__audit.password_history._${v_kevin_id}" 0 
    ...
    ${cli_dir}/sbin/start-cli.sh  -u ${v_audit_user} -h "${query_ip}" -e "show devices root.__audit.password_history.*;" >${cur_dir}/tmp.out
    check_res "root.__audit.password_history" 3 
