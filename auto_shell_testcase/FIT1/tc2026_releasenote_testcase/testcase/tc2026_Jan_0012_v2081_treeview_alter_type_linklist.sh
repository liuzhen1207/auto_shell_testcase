...
# 单个timeseries类型修改函数（动态切换类型文件）
exec_alter_type() {
    local v_ts_name_orig=$1
    local v_ts_type_initial=$2  # 初始类型（原始类型）
    local current_type="${v_ts_type_initial}"  # 跟踪当前类型，初始为原始类型

    echo "[$(date +%F_%T)] 开始处理时序：${v_ts_name_orig}，初始类型：${current_type}" | tee -a ${log_file}

    # 循环读取目标类型：直到当前类型的txt文件不存在/无有效目标类型
    while true; do
        local type_file="${cur_dir}/${current_type}.txt"

        # 检查类型文件是否存在
        if [ ! -f ${type_file} ]; then
            echo "[$(date +%F_%T)] INFO: ${v_ts_name_orig} 当前类型${current_type}无映射文件(${type_file})，停止修改" | tee -a ${log_file}
            break
        fi

        # 读取当前类型文件的第一个有效目标类型（只取第一个，改完后切换类型）
        local v_dest_type=$(grep -v '^$' ${type_file} | head -n 1)
        # 无有效目标类型则退出循环
        if [ -z "${v_dest_type}" ]; then
            echo "[$(date +%F_%T)] INFO: ${v_ts_name_orig} 当前类型${current_type}的映射文件无有效目标类型，停止修改" | tee -a ${log_file}
            break
        fi

        echo "[$(date +%F_%T)] 开始修改: ${v_ts_name_orig} ${current_type} -> ${v_dest_type}" | tee -a ${log_file}
        # 执行alter命令，捕获输出和退出码
        ...
        if [ ${exit_code} -eq 0 ]; then
            echo "[$(date +%F_%T)] SUCCESS: ${v_ts_name_orig} 从${current_type}修改为${v_dest_type}成功" | tee -a ${log_file}
            # 修改成功：更新当前类型为新类型，继续下一轮循环
            current_type="${v_dest_type}"
        else
            echo "[$(date +%F_%T)] FAIL: ${v_ts_name_orig} 从${current_type}修改为${v_dest_type}失败（退出码：${exit_code}）" | tee -a ${log_file}
            # 修改失败：终止该时序的后续修改
            break
        fi
    done

    echo "[$(date +%F_%T)] 结束处理时序：${v_ts_name_orig}，最终类型：${current_type}" | tee -a ${log_file}
    return 0
}
