...
do
   ...
   if [[ ${v_dn_pid} -gt 0 ]];then
      ...
      if [[ ${t_elp} -gt ${max_wait_time} ]];then
         ...
         do
             v_jstack_time=`date "+%Y_%m_%d_%H_%M_%S"`
             ssh ${os_user_name}@${v_dn_ip} "sudo jstack -l ${v_dn_pid} >${upd_from_db_dir}/cn_${v_jstack_num}_${v_jstack_time}.out"
             sleep 10
         done
...
do
	      ...
	      ssh ${os_user_name}@${line} "source /etc/profile;cd ${upd_from_db_dir};sudo ./sbin/stop-confignode.sh"
...
# start new node 
      ...
      ssh ${os_user_name}@${line} "source /etc/profile;cd ${upd_from_db_dir};sudo ./sbin/start-confignode.sh -H ${upd_from_db_dir}/cn_${v_after_upd_start_time}_heapdump.hprof > /dev/null 2>&1 &"
      sleep 5
      ...
      if [[ ${v_elp_time} -gt 180 ]];then
         let fail_flag++
         echo "upgrade ${line} failed."
	 v_warnMessage="${v_warnMessage}upgrade ${line} failed."
         return
      fi
      done
...
# copy lib
if ssh ${os_user_name}@${line} test -d ${upd_from_db_dir}/lib_orig; then
ssh ${os_user_name}@${line} "sudo cp -rp ${db_dir}/lib_orig ${upd_from_db_dir}/lib"
fi
done
}
...
while read line<&3
do
   ssh ${os_user_name}@${line} "sudo gunzip ${upd_from_db_dir}/logs/*confignode*all*"
   v_npe=`ssh ${os_user_name}@${line} "grep NullPointer ${upd_from_db_dir}/logs/*confignode*all*|wc -l"`
   v_cn_err1=`ssh ${os_user_name}@${line} "grep BufferUnderflowException ${upd_from_db_dir}/logs/*confignode*all*|wc -l"`
   v_cn_err2=`ssh ${os_user_name}@${line} "grep \"but return HAS_MORE_STATE\" ${upd_from_db_dir}/logs/*confignode*all*|wc -l"`
   v_cn_err3=`ssh ${os_user_name}@${line} "grep \"ClassCastException\" ${upd_from_db_dir}/logs/*confignode*all*|wc -l"`
   if [[ ${v_npe} -gt 0 ]];then
	   let fail_flag++
	   let backup_log_flag++
