...
do
  if ssh ${os_user_name}@${line} test -d ${db_dir}/logs; then
     ...
     ssh ${os_user_name}@${line} "source /etc/profile;sudo mv ${db_dir}/data ${db_dir}/data_${desc}"
     ssh ${os_user_name}@${line} "source /etc/profile;sudo mv ${data1_dir}/ ${data1_dir}_${desc}"
     ssh ${os_user_name}@${line} "source /etc/profile;sudo mv ${data2_dir}/ ${data2_dir}_${desc}"
  fi
