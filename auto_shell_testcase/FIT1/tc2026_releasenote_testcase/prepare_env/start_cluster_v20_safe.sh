...
query_host_ssh=`head -1 ${cur_dir}/../conf/datanode.txt`
...
while read d_node <&4
do
        ssh ${os_user_name}@${d_node} "source /etc/profile;cd ${db_dir};sudo ./sbin/start-datanode.sh -H ${db_dir}/dn_${v_start_time}_heapdump.hprof > /dev/null 2>&1 &"
        sleep 2
done
# continue
